"""R2 pass with ordered internal replay for action head and proprio only.
...
from refine_auxiliary_block import calibrate_auxiliary_block_ordered
...
def validate_recipe(recipe, plan, requests):
    ...
    if recipe['candidate'] != 'R2_aux_ordered':
        raise ValueError('Only the ordered-auxiliary R2 candidate is allowed')
    expected_row_mode = 'llm_action_4_context_4'
...
def run_pass(policy, bank, requests, plan, recipe, *, template, run,
    ...
    try:
        for index, (block_name, rows) in enumerate(plan['blocks'].items()):
            calibrator = (calibrate_auxiliary_block_ordered
                          if block_name in ('action_head', 'proprio_projector')
                          else calibrate_block)
            report = calibrator(policy, bank, block_name, requests,
        ...
        result = {'complete': True, 'candidate': recipe['candidate'],
                  'auxiliary_replay': 'ordered_current_linear_expert_only',
