#!/usr/bin/env python3
"""Build the ordered-auxiliary R2 candidate and verify native reload.
...
from pass_engine_ordered import run_pass
...
def recipe(candidate: str, pass_id: str, provenance: dict) -> dict:
    if candidate != 'R2_aux_ordered':
        raise ValueError('Only the ordered-auxiliary R2 candidate is frozen')
    return {
        "candidate": candidate,
        "row_mode": "llm_action_4_context_4",
        "pass_id": pass_id,
        "pool": pass_id,
...
def main() -> None:
    ...
    parser.add_argument("--candidate", choices=("R2_aux_ordered",), required=True)
