...
RUN = common.RUN.parent / 'interface-offline-attempt-01'
...
def worker(gpu):
    ...
    if int(cfg.EVALUATION.replan_steps) != plan['executed_action_prefix']:
            ...
            with torch.inference_mode():
                for suite in ((owner,) if per_expert else common.SUITES):
                    for spec, record in records[suite]:
                        ...
                        executed = common.metrics(torch, action[..., :5, :], target[..., :5, :])
                        ...
                        rows.append({'arm': arm, 'suite': suite, 'base': base, 'donor': donor,
                            ...
                            'action': full, 'executed5_action': executed, 'flow': flow,
    ...
    for arm in ARMS:
        ...
        for suite in (*common.SUITES, 'all'):
            ...
            summary[arm][suite] = {'requests': len(chosen),
                ...
                'executed5_continuous6_mse': sum(r['executed5_action']['continuous6_mse'] for r in chosen) / len(chosen),
