...
def source_hashes() -> dict[str, str]:
    return {name: sha(HERE / name) for name in
            ("freeze_eval_bank.py", "run_eval_job_v6.py", "audit_eval_job.py",
             "run_eval_queue_v6.py")}
...
def prepare() -> dict:
    ...
    plan = {"schema": "fastwam_tcr_eval_queue_v1", "host": socket.gethostname(),
            ...
            "technical_recovery_of": "evaluation-queue-v5 first worker failed before model load because its worker referenced build-v1"}
            ...
            while pending or active:
                ...
                if stop_latch:
                    ...
                    for gpu in sorted(GPUS, key=lambda i: rows[i]["free_mib"], reverse=True):
                        ...
                        log = log_path.open("x")
                        command = [str(PYTHON), "-u", str(HERE / "run_eval_job_v6.py"),
