...
def export_native(torch, model, output: Path, provenance: dict) -> dict:
    ...
    del payload
    gc.collect()
    check = torch.load(output, map_location="cpu", weights_only=True, mmap=True)
    for component in ("mot", "proprio_encoder"):
        current = getattr(model, component).state_dict()
...
def run(args: argparse.Namespace) -> dict:
    ...
    from block_calibration import calibrate_group
    ...
    for name in ("spatial", "object", "goal", "long"):
        ...
        experts[name] = torch.load(source, map_location="cpu", weights_only=True, mmap=True)
