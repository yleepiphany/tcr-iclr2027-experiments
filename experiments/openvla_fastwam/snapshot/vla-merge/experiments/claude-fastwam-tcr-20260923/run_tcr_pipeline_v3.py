...
def code_hashes() -> dict[str, str]:
    names = ("audit_ab_bank.py", "audit_capture.py", "block_calibration_v2.py",
             "build_two_pass_v3.py", "capture_closed_loop.py", "check_real_prefix_parity_v2.py",
             "freeze_bank.py", "freeze_block_plan.py", "native_trace.py",
             "replay_merged_prefix.py", "replay_native_trace.py",
             "run_capture_episode.py", "run_capture_queue.py", "run_tcr_pipeline_v3.py")
...
def prepare() -> dict:
    ...
    plan = {"schema": "fastwam_tcr_pipeline_v1", "host": socket.gethostname(),
            ...
            "success_evaluations": 0, "recovery_of": "pipeline-v2 root proprio tensor-key audit bug; prior v1 mmap compatibility failure",
    ...
    with (RUN / "coordinator.lock").open("a+") as lock_file:
        ...
        try:
            ...
            try:
                ...
                parity_path = BASE / "real-prefix-parity-v3.json"
                if parity_path.exists():
                    raise FileExistsError(parity_path)
                parity_command = [str(PYTHON), "-u", str(HERE / "check_real_prefix_parity_v2.py"),
                ...
                validate_frozen(plan)
                build_output = BASE / "tcr-build-v3"
                if build_output.exists():
                    raise FileExistsError(build_output)
                build_command = [str(PYTHON), "-u", str(HERE / "build_two_pass_v3.py"),
