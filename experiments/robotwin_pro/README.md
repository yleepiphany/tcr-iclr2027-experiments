...
The [RoboTwin RegMean++ M=3 port](native_sources/experiments/robotwin-regmeanpp-m3-codex-20260925/README.md) now supplies that distinct 67-stage merged-prefix graph materializer and a [frozen CPU plan](evidence/ROBOTWIN-REGMEANPP-M3-CPU-PLAN.json). Ten CPU tests check three-expert row budgets, block input identity, atomic updates and the unchanged spectral-smoothing kernel; native GPU parity has **not** yet passed. Separate [FeatCal](native_sources/vla-merge-runtime/experiments/robotwin-static-baselines-readiness-20260925/safe-successor-v1/README.md) and [RegMean++](native_sources/experiments/robotwin-regmeanpp-m3-codex-20260925/native_smoke_supervisor_v1/README.md) one-shot supervisors are waiting for an empty, leased GPU to attempt only their respective native parity smokes. Their [launch receipts](evidence/ROBOTWIN-FEATCAL-M3-SMOKE-LAUNCH.json) and [RegMean++ launch receipt](evidence/ROBOTWIN-REGMEANPP-M3-SMOKE-LAUNCH.json) identify these waiting supervisors. A smoke is not a model build or a scored RoboTwin episode.
