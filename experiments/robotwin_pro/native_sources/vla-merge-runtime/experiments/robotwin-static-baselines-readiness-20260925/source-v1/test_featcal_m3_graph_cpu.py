"""Small CPU graph tests of the M3 adapter's real hook/index/prefix components.
...
from featcal_m3 import API,GROUPS,TOTAL_ROWS

WORK=Path(os.environ.get('ROBOTWIN_TEST_WORK','/mnt/workspace/Wilson/parameter-fusion'))
BANK=WORK/'vla-merge-runtime/experiments/robotwin-static-baselines-readiness-20260925/static-bank-v1/bank.json'
...
class Tests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        torch.set_num_threads(2);cls.api=API(WORK,BANK);cls.steps=cls.api.steps
        ...
        cls.index=cls.api.cache.build_shard_index(cls.quotas,cls.steps,cls.api.cache.build_call_slots(),contract_sha256='test-plan',
            expert_model_sha256={g:'test-'+g for g in GROUPS},initial_student_sha256='a'*64)
    ...
    def teacher(self,model):
        ...
        for replica in range(3):
            c=self.api.cache.MultiLayerStreamingRowCollector(model,self.steps,self.quotas,
                call_slot_ids=self.api.slots('coordination',0,replica),call_ordinals_within_task=[r*3+replica for r in range(5)],
    ...
    def student_step(self,model,step,teacher_keys):
        ...
        for replica in range(3):
            c=self.api.SingleStepSelectedRowCollector(model,step,qs,teacher_row_keys=teacher_keys,
                call_slot_ids=self.api.slots('coordination',0,replica),call_ordinals_within_task=[r*3+replica for r in range(5)],row_mask_provider=masks)
            result=c.capture(model)
    ...
    def test_47_stage_418_weight_450_call_2820_shard_identity(self):
        self.assertEqual(len(self.steps),47);self.assertEqual(len(self.quotas),418)
        self.assertEqual(len(self.api.cache.build_call_slots()),450);self.assertEqual(self.index['shard_count'],2820)
        self.assertEqual(sum(q.rows_per_expert for q in self.quotas)*3,TOTAL_ROWS)
    ...
    def test_original_exact_solver_consumes_m3_shards_and_commits_atomically(self):
                ...
                for group in GROUPS:
                    for task in range(10):
                        ...
                        self.api.cache.write_completed_shard_atomic(cache,spec,tensors,plan_sha256='test-plan',
                            call_state_manifest_sha256=self.api.bank_sha,
            ...
            with ExitStack() as stack:
                ...
                with patch.object(b,'featcal_hybrid_linear_weight',side_effect=cpu_kernel):
                    prefix,previous,receipt=b._solve_formal_step(cache_root=cache,student=model,step=step,
                        ...
                        soup_handle=sh,base_handle=bh,expert_handles={g:eh for g in GROUPS})
