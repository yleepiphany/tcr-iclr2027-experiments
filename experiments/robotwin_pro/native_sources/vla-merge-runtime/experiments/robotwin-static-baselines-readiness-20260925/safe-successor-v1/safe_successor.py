#!/usr/bin/env python3
"""Single-card, empty-card-only successor for frozen RoboTwin FeatCal M3.
...
EXPECTED_PLAN_SHA='f9fbc255cf37f5bec14fc2ba36d1b7c147b2d7205e6999f3a6a007df331c7eb5'
MIN_FREE_MIB=49152
...
def paths(work):
    ...
    return root,root/'source-v1',root/'static-bank-v1/bank.json',root/'featcal-m3-cpu-plan-v1'

def preflight(work):
    root,source,bank,run=paths(work);plan=read(run/'plan.json')
    if sha(run/'plan.json')!=EXPECTED_PLAN_SHA:raise ValueError('Frozen RoboTwin M3 plan changed')
    ...
    return {'status':'CPU_PREFLIGHT_PASS_NATIVE_GPU_NOT_RUN','plan':str(run/'plan.json'),'plan_sha256':EXPECTED_PLAN_SHA,
...
def worker(args):
    ...
    if owner['plan_sha256']!=EXPECTED_PLAN_SHA or owner['runner_sha256']!=sha(__file__) or owner['target']!=args.target:
    ...
    def checked(*a,**kw):
        api.check_runtime_memory();value=native(*a,**kw);api.check_runtime_memory();return value
    ...
    if not (run/'native-smoke.json').exists():
        ...
        with stage_guard(run,'smoke'):api.smoke(run)
    api.require_smoke(run,digest);event(queue,'native_smoke_pass',receipt=str(run/'native-smoke.json'))
    ...
    for group in ('coordination','receptacle','precision'):
        ...
        if marker.exists():
            ...
            with stage_guard(run,'teachers',group):api.teachers(run,group)
    event(queue,'sequential_solve_started',stages=47)
    with stage_guard(run,'solve'):api.solve(run)
            ...
            for gpu in args.gpus:
                try:
                    with acquire_empty_card(args.workspace_root,gpu,queue) as held:
                        ...
                        admission={**held['snapshot'],'controller_pid':os.getpid(),'plan_sha256':EXPECTED_PLAN_SHA,
                        ...
                        env={**os.environ,'CUDA_VISIBLE_DEVICES':str(gpu),'OMP_NUM_THREADS':'4','MKL_NUM_THREADS':'4'}
