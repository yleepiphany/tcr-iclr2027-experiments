#!/usr/bin/env python3
"""Isolated native RoboTwin TCR formal panel after support packaging recovery."""
...
BUILD = BASE / "tcr-build-attempt-02"
OLD_FORMAL = BASE / "tcr-formal-attempt-02"
MODELS = BASE / "tcr-checkpoints-v2"
RUN = BASE / "tcr-formal-attempt-03"
...
def prepare() -> dict:
    ...
    previous = read(OLD_FORMAL / "queue-ended.json")
    require(previous.get("complete") is False and not previous.get("accepted")
            and not previous.get("failed") and not previous.get("pending")
            and len(previous.get("skipped", {})) == 9,
            "Previous formal queue did not cleanly skip all nine jobs after build failure")
    transfer = read(TRANSFER)
    expected_ids = {f"r{repeat:02d}-{group}" for repeat in REPEATS for group in GROUPS}
    require(transfer.get("accepted") is True
            and transfer.get("release_id") == "ROBOTWIN-TRANSFER-201-RELEASE"
            and set(transfer.get("released_formal_jobs", [])) == expected_ids,
            "RoboTwin formal transfer is not accepted")
    ...
    plan = {"schema": "robotwin_tcr_three_repeat_formal_v2",
            ...
            "previous_formal_terminal_sha256": sha(OLD_FORMAL / "queue-ended.json"),
...
def validate_plan(plan: dict) -> None:
    require(plan.get("schema") == "robotwin_tcr_three_repeat_formal_v2"
            ...
            and plan.get("transfer_accept_sha256") == sha(TRANSFER)
            and plan.get("episodes") == 540 and len(plan.get("jobs", [])) == 9,
            "Frozen RoboTwin TCR formal plan differs")
    require(plan.get("previous_formal_terminal_sha256") == sha(OLD_FORMAL / "queue-ended.json"),
            "Previous formal failure provenance changed")
