...
def per_trace_row_indices(nrows: int, cap: int, flow: int, request_slot: int,
                          *, cameras: int = 1, camera: int = 0,
                          last_only: bool = False) -> list[int]:
    """Match the frozen RoboTwin B per-replay cap, not Table-3's request quota."""
    if (nrows < 1 or cap != 16 or flow not in (0, 5, 9)
            or request_slot not in range(5) or cameras not in (1, 3)
            or not 0 <= camera < cameras or last_only):
        raise ValueError("RoboTwin B row-selection arguments differ")
    if nrows <= cap:
        return list(range(nrows))
    import torch
    return torch.linspace(0, nrows - 1, cap).round().long().tolist()
...
def main() -> None:
    ...
    if which == "B":
        # The generic ablation path spreads one cap across flow/view siblings.
        # RoboTwin's frozen B contract instead caps every collected replay
        # state, exactly as the non-ablation A path's sample_rows() does.
        table3_contract.row_indices = per_trace_row_indices
