#!/usr/bin/env python3
"""Isolated RoboTwin TCR build recovery retaining the frozen Soup deployment support."""
...
RUN = BASE / "tcr-build-attempt-02"
OLD_RUN = BASE / "tcr-build-attempt-01"
...
SOLVER = HERE / "run_materializer_local_v2.py"
...
def failure_gate() -> None:
    terminal = read(OLD_RUN / "queue-ended.json")
    a = {f"r{repeat:02d}-passA" for repeat in (1, 2, 3)}
    b = {f"r{repeat:02d}-passB" for repeat in (1, 2, 3)}
    require(terminal.get("complete") is False and not terminal.get("accepted")
            and not terminal.get("pending") and set(terminal.get("failed", {})) == a
            and set(terminal.get("skipped", {})) == b,
            "RoboTwin first build attempt did not fail at the three pass-A packaging steps")
    for job in a:
        receipt = read(OLD_RUN / "exits" / f"{job}.json")
        log = (OLD_RUN / "logs" / f"{job}.log").read_text(errors="replace")
        require(receipt.get("returncode") == 1 and receipt.get("accepted") is False
                ...
                and "Support file policy_preprocessor_step_3_normalizer_processor.safetensors differs for precision" in log,
                f"{job}: first-attempt failure differs from support packaging mismatch")
...
def prepare() -> dict:
    if RUN.exists() or MODELS.exists():
    ...
    plan = {"schema": "robotwin_three_expert_two_pass_build_v1",
            ...
            "failed_attempt_terminal_sha256": sha(OLD_RUN / "queue-ended.json"),
...
def validate_plan(plan: dict) -> None:
    ...
    failure_gate()
    require(plan.get("failed_attempt_terminal_sha256") == sha(OLD_RUN / "queue-ended.json")
