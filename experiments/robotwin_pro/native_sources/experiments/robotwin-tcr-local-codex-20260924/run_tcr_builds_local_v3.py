#!/usr/bin/env python3
"""Isolated RoboTwin TCR recovery reusing the completed pass-A model artifacts."""
...
SOLVER = HERE / "run_materializer_local_v3.py"
...
def failure_gate() -> None:
    ...
    require(terminal.get("complete") is False and not terminal.get("accepted")
            ...
            "RoboTwin second build attempt did not end with three missing-telemetry A jobs")
    for job in a:
        receipt = read(OLD_RUN / "exits" / f"{job}.json")
        require(receipt.get("returncode") == 0 and receipt.get("accepted") is False
                and receipt.get("error", "").startswith("artifact gate: FileNotFoundError:")
                and f"telemetry/{job}.json" in receipt["error"]
                and not (OLD_RUN / "telemetry" / f"{job}.json").exists(),
                f"{job}: second-attempt failure differs from missing peak telemetry")
