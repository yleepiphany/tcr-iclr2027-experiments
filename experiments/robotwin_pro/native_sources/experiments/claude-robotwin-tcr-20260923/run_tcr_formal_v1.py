#!/usr/bin/env python3
"""Frozen native RoboTwin TCR formal panel; three repeats, nine 60-episode jobs."""
...
RUN = BASE / "tcr-formal-attempt-01"
...
def expert_reset_hashes(source, group: str, repeat: int) -> tuple[dict, dict]:
    ...
    binding = terminal["receipt"]
    receipt_path = Path(binding["path"])
    require(sha(receipt_path) == binding["sha256"],
    ...
    hashes = {}
    for binding in receipt["rows"]:
        path = Path(binding["path"])
        require(sha(path) == binding["sha256"], "Matched Experts raw row changed")
        row = read(path)
    ...
    return hashes, {"terminal_path": str(terminal_path), "terminal_sha256": sha(terminal_path),
                    "receipt_path": str(receipt_path), "receipt_sha256": binding["sha256"]}
...
def prepare() -> dict:
    ...
    plan = {"schema": "robotwin_tcr_three_repeat_formal_v1",
...
def validate_plan(plan: dict) -> None:
    require(plan.get("schema") == "robotwin_tcr_three_repeat_formal_v1"
