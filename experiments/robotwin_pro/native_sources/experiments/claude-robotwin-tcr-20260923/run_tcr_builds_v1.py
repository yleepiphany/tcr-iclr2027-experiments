#!/usr/bin/env python3
"""Frozen three-repeat RoboTwin A(relative) -> B(uniform) TCR build queue."""
...
RUN = BASE / "tcr-build-attempt-01"
MODELS = BASE / "tcr-checkpoints-v1"
AUDIT = WORK / "coordination/2026-09-23/robotwin-capture-combined-v3-final-audit.json"
PROTOCOL = BASE / "reset-preflight-attempt-03/candidate-protocol.json"
BANK = BASE / "dense-experts-v2/expert-dense-bank.json"
SOUP = BASE / "dense-soup-v1/COMPLETE.json"
SOLVER = HERE / "run_materializer.py"
...
def trace_path(report: dict) -> Path:
    return BASE / f"capture-v{2 if report['attempt'] == '02' else 3}/jobs" / report["id"]
...
def prepare() -> dict:
    ...
    plan = {"schema": "robotwin_three_expert_two_pass_build_v1",
            ...
            "source_sha256": {str(path): sha(path) for path in (SOLVER, GENERIC,
                                HERE / "robotwin_solver_contract.py", Path(__file__))},
