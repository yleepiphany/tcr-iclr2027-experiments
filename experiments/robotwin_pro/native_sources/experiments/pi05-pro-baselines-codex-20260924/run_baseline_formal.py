#!/usr/bin/env python3
"""Serial formal PRO baseline queue on local GPU 7, with shared-card admission."""
...
METHOD = POLICY_SHA = SELECTION_MANIFEST_SHA = None
SPECS = {
    "ties_merging": {
        "model": RUNTIME / "experiments/iclr2027-table1-20260910/libero/ta-ties-fastlane-v2/candidates/ties_density_0p3_alpha_0p9/pretrained_model",
        "model_sha": "9382c79c9a76dea4803d71484599ec16edb77ee3cf7ce3b861fb9f0296921847",
        "selection_sha": "e451b150756e302201a1cce736eb04e5abaf20d228561ca70119922262807e5e",
    },
    "regmean_pp": {
        "model": RUNTIME / "experiments/iclr2027-table1-20260910/libero/original-regmeanpp/repeat-01/merge/attempt-03-active-support-v2-host1023-gpu2",
        "model_sha": "38745ba812c2f4ac4327ab40719e0ce8da017873c0a1765608f65dd926cd4ab3",
        "selection_sha": "61490ca165af7ad60ba33e6f3ab1b569a681db7ebb12b4c5f29e492cfc9c33b2",
    },
    "featcal": {
        "model": RUNTIME / "experiments/iclr2027-table1-20260910/preflight/featcal-formal-full-v1/checkpoint/pretrained_model",
        "model_sha": "e43de109844431c02e316e57f701d7c06a9a3c8feaa3c41c9f391281f8197efa",
        "selection_sha": "cbd455255be21186db59117cc24eec2672e0fa598e6d147407a07a347600d1b0",
    },
}
...
def select_method(method: str) -> None:
    global SELECTION, RUN, POLICY, METHOD, POLICY_SHA, SELECTION_MANIFEST_SHA
    spec = SPECS[method]
    SELECTION = ROOT / method / "selection-v1"
    RUN = ROOT / method / "formal-v1"
    POLICY = spec["model"]
    METHOD = f"{method}-main10k-codex-v1"
    POLICY_SHA = spec["model_sha"]
    SELECTION_MANIFEST_SHA = spec["selection_sha"]
...
def validate_source() -> None:
    ...
    if sha256_file(SELECTION / "manifest.json") != SELECTION_MANIFEST_SHA:
        raise ValueError("Frozen PRO baseline selection manifest changed")
    if sha256_file(POLICY / "model.safetensors") != POLICY_SHA:
        raise ValueError("Frozen main-table baseline checkpoint changed")
...
def prepare() -> dict:
    ...
    for repeat in REPEATS:
        ...
        if len(native_plan["commands"]) != 160 or len(selection.jobs) != 160:
            raise ValueError(f"Missing formal PRO baseline jobs: {repeat}")
    ...
    if len(jobs) != 480 or len({job["id"] for job in jobs}) != 480:
        raise ValueError("PRO baseline must contain exactly 480 distinct jobs")
    if sum(len(job["state_ids"]) for job in jobs) != 4800:
        raise ValueError("PRO baseline must contain exactly 4800 episodes")
    plan = {"schema": "pi05_pro_baseline_formal_shared_v1", "created_at": now(),
    ...
    write(RUN / "CLAIM.json", {"owner": "Codex", "host": HOST,
                               "scope": f"pi0.5 PRO {METHOD} exact main-table 4800 formal episodes",
...
def verify_job(job: dict, plan: dict) -> dict:
    ...
    for key, value in expected.items():
        if receipt.get(key) != value:
            raise ValueError(f"PRO baseline receipt {job['id']} mismatched {key}")
    rows = info.get("per_task")
    if not isinstance(rows, list) or len(rows) != 1 or rows[0].get("task_id") != job["task_id"]:
        raise ValueError(f"PRO baseline task coverage differs: {job['id']}")
    ...
    if (not isinstance(successes, list) or len(successes) != 10
        ...
        raise ValueError(f"PRO baseline lacks ten boolean outcomes: {job['id']}")
    overall = info.get("overall") or {}
    if (overall.get("n_episodes") != 10
            or abs(float(overall.get("pc_success", -1)) - 10 * sum(successes)) > 1e-6):
        raise ValueError(f"PRO baseline overall disagrees with raw outcomes: {job['id']}")
...
def run() -> None:
    ...
    if (plan["schema"] != "pi05_pro_baseline_formal_shared_v1" or plan["host"] != HOST
            or plan["gpu"] != GPU or plan["gpu_uuid"] != UUID
            or plan["source_manifest_sha256"] != SELECTION_MANIFEST_SHA
            or plan["model_sha256"] != POLICY_SHA or plan["method"] != METHOD
            or plan["jobs_expected"] != 480 or plan["episodes_expected"] != 4800
            or len(plan["jobs"]) != 480
    ...
    validate_source()
    lock_path = ROOT / "gpu-7-baseline-shared-lane.lock"
    handle = os.open(lock_path, os.O_CREAT | os.O_RDWR, 0o644)
    try:
        fcntl.flock(handle, fcntl.LOCK_EX | fcntl.LOCK_NB)
    except BlockingIOError:
        os.close(handle)
        raise RuntimeError("Another local PRO baseline shared lane holds GPU 7")
    admitted = row()
