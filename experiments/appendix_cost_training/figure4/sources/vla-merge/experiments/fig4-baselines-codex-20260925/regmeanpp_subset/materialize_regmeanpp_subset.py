#!/usr/bin/env python3
"""Disclosed spectral-smoothing extension to the original full-418 RegMean++ Gram.

The original input/block protocol and alpha .3 are fixed. The inverse is changed
to lambda/(lambda^2+tau^2) on the arithmetic-mean-centered correction, with
tau=d*eps(float32)*estimated lambda_max(A). This is an engineering regularizer,
not an equivalent numerical implementation or an official VLA implementation.

This generalizes the two-expert Spatial/Object solver to NAME=checkpoint
experts.  Each LoRA expert is first treated as a dense full-weight expert:

    W_i = exact frozen dense checkpoint tensor, converted to float32 for solve

Every target retains the released off-diagonal-scaled Gram LHS/RHS and equal
expert mass but uses the explicitly disclosed weak-spectrum smooth filter.
There are no teacher-output targets, Soup ridge, correction caps or clipping. Vision, Language and Action blocks are merged in graph
order. Each newly merged complete block generates the next block's inputs.
"""
...
import os
import sys
sys.path.insert(0, "/mnt/workspace/Wilson/parameter-fusion/vla-merge/scripts")
sys.path.insert(0, "/mnt/workspace/Wilson/parameter-fusion/vla-merge/experiments/static-observation-baselines-20260922/regmeanpp_spectral_smoothing_v1")

import eval_with_local_tokenizer  # noqa: F401
from vla_merge.original_regmeanpp_v2 import ORIGINAL_EXPERT_ORDER
...
_unfiltered_load_replay_states = load_replay_states

def load_replay_states(tensor_path, manifest_path, expected_task, device,
                       max_states=0, required_flow_index=None):
    """Read only stored first-step Gaussian inputs; never later teacher flow states."""
    from collections import Counter
    from fig4_regmeanpp_contract import SCHEMA, digest
    if max_states != 0 or required_flow_index not in (None, 0):
        raise ValueError("Static arm disallows further subsampling or denoised flow states")
    states, source = _unfiltered_load_replay_states(
        tensor_path, manifest_path, expected_task, device, 0, 0)
    if source.get("source_kind") != "demo_observation_expert_generation" or source.get("demonstration_actions_used") is not False:
        raise ValueError("Source must be observation-only demonstration capture")
    if source.get("paired_noise_verified") != {"flow0_pairs_checked": 50, "flow0_equal": 50, "max_abs_diff": 0.0}:
        raise ValueError("Saved first-step noise lacks audited tensor equality")
    samples = [dict(item) for item in source["samples"] if item["flow_index"] == 0]
    if len(states) != 50 or len(samples) != 50:
        raise ValueError("Static observation count differs")
    # Preserve native record indices so provenance stays mapped to original tensors.
    manifest = dict(source)
    manifest.update(samples=samples, sample_count=50, samples_per_episode=5,
        flow_indices=[0], source_kind="static_demo_observation_pure_noise_t1",
        static_repair_schema=SCHEMA, prompt_sample_counts=dict(Counter(str(s["prompt_signature"]) for s in samples)),
        source_manifest_sha256=digest(manifest_path),
        selected_source_indices=[s["index"] for s in samples],
        source_manifest_path=str(manifest_path), native_denoising_trajectory_used=False,
        environment_rollout_calibration_used=False)
    return states, manifest


EXPECTED_UPSTREAM_COMMIT = "a9fdd97629707062a5872653dd9b34f7e25c7243"
EXPECTED_UPSTREAM_SOURCE_SHA256 = "2fab407dd9f19f12fca482a2a2e5ddcc65715a9b728ba60e64d2b163b27f4803"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--official-regmeanpp-repo",
        type=Path,
        default=Path(
            "/mnt/workspace/Wilson/parameter-fusion/vla-merge-runtime/references/RegMean-plusplus"
        ),
    )
    ...
    parser.add_argument("--offdiag-scale", type=float, default=0.3)
...
def main() -> None:
    args = parse_args()
    from fig4_regmeanpp_contract import validate_dense_bank, validate_recipe, validate_realized_rows, validate_trace
    experiment_plan = validate_recipe(args)
    ORIGINAL_EXPERT_ORDER = tuple(experiment_plan["experts"])
    forbidden_variant = (
        args.prior_model is not None
        or args.ridge_ratio != 0.0
        or args.max_correction_ratio != 0.0
        or args.replay_prefix != "merged"
        or args.expert_loss_normalization != "none"
        or args.expert_aggregation != "mean"
        or args.expert_weight_plan is not None
        or args.objective_grouping != "expert"
        or args.require_objective_improvement
        or args.freeze_prefix_from_prior
        or args.freeze_action_interface_from_prior
        or args.action_block_solver != "independent_linear"
        or bool(args.calibration_source_weight)
    )
    if forbidden_variant:
        raise ValueError(
            "Original RegMean++ forbids prior/ridge/cap/reweighting/gates/frozen scope "
            "and requires merged-prefix independent-Linear replay"
        )
    experiment_manifest_path = args.experiment_manifest.expanduser().resolve()
    upstream_repo = args.official_regmeanpp_repo.expanduser().resolve()
    upstream_source = upstream_repo / "fusion_bench/method/regmean_plusplus/regmean.py"
    upstream_commit = subprocess.check_output(
        ["git", "-C", str(upstream_repo), "rev-parse", "HEAD"], text=True
    ).strip()
    if (
        upstream_commit != EXPECTED_UPSTREAM_COMMIT
        or sha256(upstream_source) != EXPECTED_UPSTREAM_SOURCE_SHA256
    ):
        raise ValueError("Official RegMean++ source identity differs")
    if (
    ...
    if tuple(experts) != ORIGINAL_EXPERT_ORDER:
        raise ValueError(
            "Subset RegMean++ requires the exact frozen selected-expert order"
        )
    ...
    if device.type == "cuda":
        fraction=float(os.environ.get("REGMEAN_ALLOCATOR_FRACTION", "0.70"))
        if not 0 < fraction <= .70:
            raise ValueError("Allocator cap must be in (0, .70]")
        ...
        for spec in specs:
            def hook(_module: torch.nn.Module, inputs: tuple[Any, ...], module_name: str = spec.base_name) -> None:
                ...
                captures[module_name].setdefault(current_label, []).append(
                    sample_rows(inputs[0], args.max_rows_per_sample) * current_sqrt_weight
                )

            handles.append(spec.module.register_forward_pre_hook(hook))
        try:
            for forward, label, sample_weight in zip(
    ...
    def solve_spec(
        ...
        return solve_smoothing_adapter(
            grouped_inputs,
            grouped_weights,
        ...
        for name in names:
            for label, value in x_by_expert[name].items():
                if label in grouped_inputs:
                    raise ValueError(f"Duplicate objective-group label: {label}")
                grouped_inputs[label] = value
                grouped_weights[label] = expert_weights[name]
        merged_weight, diagnostics = solve_smoothing_adapter(
            grouped_inputs,
            grouped_weights,
            prior_weight.to(device),
            offdiag_scale=args.offdiag_scale,
        )
        merged_bias = sum(expert_biases.values()) / len(expert_biases)
        diagnostics["bias_semantics"] = "uniform_expert_mean_not_augmented_regression"
        return merged_weight, merged_bias.detach().cpu(), diagnostics
    ...
    def grouped_state_rows(name: str, value_fn: Any) -> dict[str, torch.Tensor]:
        values = [
            sample_rows(value_fn(state), args.max_rows_per_sample) * math.sqrt(sample_weight)
    ...
    if args.freeze_action_interface_from_prior:
        ...
        action_weight, action_bias, action_metrics = solve_dense_module(
            "action_in_proj",
            {name: grouped_state_rows(name, lambda state: state.action_input) for name in names},
        )
        time_in_weight, time_in_bias, time_in_metrics = solve_dense_module(
            "time_mlp_in",
            {name: grouped_state_rows(name, lambda state: state.time_input) for name in names},
        )
        for short_name, weight, bias, module_metrics in (
            ("action_in_proj", action_weight, action_bias, action_metrics),
            ("time_mlp_in", time_in_weight, time_in_bias, time_in_metrics),
        ):
            module = getattr(core, short_name)
            module.weight.data.copy_(weight.to(device=device, dtype=module.weight.dtype))
            module.bias.data.copy_(bias.to(device=device, dtype=module.bias.dtype))
            base_name = f"model.{short_name}"
            output_tensors[f"{base_name}.weight"] = weight.to(output_tensors[f"{base_name}.weight"].dtype)
            output_tensors[f"{base_name}.bias"] = bias.to(output_tensors[f"{base_name}.bias"].dtype)
            metrics[base_name] = {"stage": "front_end", "kind": "original_regmeanpp_weight_bias_mean", **module_metrics}
        ...
        time_out_weight, time_out_bias, time_out_metrics = solve_dense_module("time_mlp_out", time_hidden_by_expert)
        core.time_mlp_out.weight.data.copy_(time_out_weight.to(device=device, dtype=core.time_mlp_out.weight.dtype))
        core.time_mlp_out.bias.data.copy_(time_out_bias.to(device=device, dtype=core.time_mlp_out.bias.dtype))
        output_tensors["model.time_mlp_out.weight"] = time_out_weight.to(output_tensors["model.time_mlp_out.weight"].dtype)
        output_tensors["model.time_mlp_out.bias"] = time_out_bias.to(output_tensors["model.time_mlp_out.bias"].dtype)
        metrics["model.time_mlp_out"] = {"stage": "front_end_sequential", "kind": "original_regmeanpp_weight_bias_mean", **time_out_metrics}
    ...
    if args.freeze_action_interface_from_prior:
        ...
        output_weight, output_bias, output_metrics = solve_dense_module(
            OUTPUT_MODULE,
            {name: grouped_state_rows(name, final_hidden) for name in names},
        )
        output_tensors["model.action_out_proj.weight"] = output_weight.to(output_tensors["model.action_out_proj.weight"].dtype)
        output_tensors["model.action_out_proj.bias"] = output_bias.to(output_tensors["model.action_out_proj.bias"].dtype)
        metrics["model.action_out_proj"] = {"stage": "output_head_sequential", "kind": "original_regmeanpp_weight_bias_mean", **output_metrics}
    ...
    manifest = {
        ...
        "method": "pi05_fig4_regmeanpp_subset_alpha03_static_t1_spectral_smoothing_v1",
        "training": False,
        "figure4_method": "RegMean++ with disclosed spectral smoothing",
        "figure4_parent_plan_sha256": experiment_plan["fig4_plan_sha256"],
        "figure4_subset": experiment_plan["subset"],
        "selected_expert_names": names,
        "calibration_observations_total": 50 * len(names),
        "experiment_manifest": {
            "path": str(experiment_manifest_path),
            "sha256": args.expected_experiment_sha256,
            "schema": experiment_plan["schema"],
        },
        "parameter_prior": "fixed alpha .3 from published small-LLM RegMean++ settings and preexisting v9; no score selection",
        "calibration_source": f"{50*len(names)} static demonstration observations plus independent initial Gaussian noise at t=1",
        "demonstration_action_labels_used": False,
        "native_denoising_trajectory_used": False,
        "environment_rollout_calibration_used": False,
        "formal_outcomes_used_for_recipe_selection": False,
        "gram_normalization": "equal rows per expert: raw XTX is common-scale equivalent to equal-mass token-normalized Gram",
        "upstream": {
        ...
        "local_prior": "no_Soup_prior; arithmetic_expert_mean_is_nullspace_continuation_for_spectral_filter",
        "calibration_policy": (
        ...
        "offdiag_scale": args.offdiag_scale,
        "solver_support_semantics": "smooth_lambda_over_lambda_squared_plus_tau_squared_on_centered_original_Gram",
        "zero_support_tie_break": "uniform_selected_expert_weight_mean",
        "soup_ridge_or_pinv_forbidden": True,
        "strict_original_equivalence": False,
        "spectral_regularization_applied": True,
        "smoothing_configuration": experiment_plan["spectral_smoothing"],
        "residual_semantics": "Smoothed-system residual verifies this extension; original-equation residual reports expected objective deviation and need not vanish",
        "bias_semantics": "uniform_expert_mean_not_augmented_regression",
        "unreported_extensions_applied": False,
