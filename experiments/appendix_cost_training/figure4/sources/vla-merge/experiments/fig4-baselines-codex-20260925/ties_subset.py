...
def prepare():
    ...
    for subset, experts in SUBSETS.items():
        ...
        jobs.append({'id': reference['id'], 'subset': subset, 'experts': experts,
                     ...
                     'build_command': [str(WORK / 'pi05_lora_finetune_v2_20260826/.venv/bin/python'), '-u', str(HERE / 'ties_subset.py'), 'build', '--subset', subset],
                     'native_load_command': [str(WORK / 'pi05_lora_finetune_v2_20260826/.venv/bin/python'), '-u', str(HERE / 'ties_subset.py'), 'native-load', '--subset', subset]})
    plan = {'schema': 'fig4_ties_subset_adapter_v1', 'created_at': stamp(), 'parent_plan_sha256': PARENT_SHA,
