...
from scripts.verify_featcal_replay_identity import baseline_oracle
from scripts.diagnose_featcal_execution_parity import native_velocity_forward
from vla_merge.featcal_execution_gate import validate_execution_identity

TABLE1 = baseline.TABLE1
PILOT = ROOT.parent / "vla-merge-runtime/experiments/featcal-execution-pilot-20260916"
BUILD = PILOT / "build-v2"
...
def implementations():
    paths = [Path(__file__), ROOT / "src/vla_merge/featcal_execution.py",
             ...
             ROOT / "scripts/verify_featcal_replay_identity.py",
             ROOT / "scripts/diagnose_featcal_execution_parity.py",
             ROOT / "scripts/run_pi05_featcal_execution_pilot.py",
             ROOT / "src/vla_merge/featcal_execution_gate.py"]
...
def diagnostic_contract():
    ...
    for name in ("identity-spatial-strict-fp32-v1", "identity-object-baseline-v1"):
        path = PILOT / "diagnostics" / name / "summary.json"
        receipt = baseline.load_json(path)
        if receipt.get("status") != "passed":
            raise ValueError("Required diagnostic did not pass")
...
def prepare():
    ...
    plan = {"schema_version": 1, "method": "FeatCal + specialist execution inputs",
            ...
            "identity_gates": {"raw_native_batch1": "bitwise equality, all 600 calls",
                               "baseline_full_joint_batch5": "bitwise equality, all 120 batches",
                               "strict_fp32_graph_parity": "independent diagnostic, atol=rtol=1e-4"},
            "diagnostics": diagnostics,
            "backend": {"tf32_override": os.environ.get("TORCH_ALLOW_TF32_CUBLAS_OVERRIDE")},
            "replaces_attempt": str(PILOT / "build"),
            "gate_revision_reason": "v1 conflated graph/dtype/batch numerical drift with raw capture identity; no score observed",
            "calibration_graph_unchanged": True,
...
def contract():
    ...
    if diagnostic_contract() != plan["diagnostics"]:
        raise ValueError("Diagnostic receipts changed after plan freeze")
    if os.environ.get("TORCH_ALLOW_TF32_CUBLAS_OVERRIDE") != plan["backend"]["tf32_override"]:
        raise ValueError("Calibration backend differs from frozen plan")
    plan_sha = sha256_file(BUILD / "plan.json")
        ...
        for expert in names:
            ...
            for task in range(10):
                ...
                for flow in FLOWS:
                    ...
                    # Validate identical computations, not different BF16/batch paths.
                    # Raw data identity: replay every request through the native b1 graph.
                    native = torch.cat([
                        native_velocity_forward(model, {k: v[i:i+1] for k, v in batch.items()})
                        for i in range(5)
                    ])
                    # Adapter identity: compare to original FeatCal's model.forward.
                    oracle = baseline_oracle(model, batch)
                    identity = validate_execution_identity(
                        native, batch["native_velocity"], output["velocity"], oracle)
                    event(event="teacher_exact_identity", expert=expert, task=task, flow=flow,
                          **identity)
                    del native, oracle
