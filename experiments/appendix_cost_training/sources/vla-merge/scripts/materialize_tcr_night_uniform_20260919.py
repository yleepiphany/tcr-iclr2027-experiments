#!/usr/bin/env python3
"""TCR-E full-418 solver using the audited PEFT-safe dense expert bank.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ablation-config", type=Path, help="Table3 is not used in this independent experiment")
    parser.add_argument("--night-reference", type=Path, required=True)
...
def main() -> None:
    ...
    if ablation is None:
        ...
        if args.expert_loss_normalization != "none":
            raise ValueError("Night uniform arm requires exactly uniform masses")
        validate_recipe(Namespace(**{**vars(args), "expert_loss_normalization": "prior"}))
        reference = load_json(args.night_reference)
        if (reference["expert_loss_normalization"] != "prior"
                or reference["realized_row_total"] != 4441200
                or len(reference["modules"]) != 418):
            raise ValueError("Reference must be the freshly built original TCR-E arm")
        if reference["dense_expert_bank_sha256"] != sha256(args.dense_expert_bank):
            raise ValueError("Expert bank differs from paired arm")
        for values, key in ((args.calibration, "calibrations"), (args.manifest, "manifests")):
            actual = {}
            for value in values:
                name, path = value.split("=", 1)
                actual.setdefault(name, []).append(str(Path(path).resolve()))
            if actual != reference["inputs"][key]:
                raise ValueError("Paired calibration inputs differ")
        actual_experts = dict(value.split("=", 1) for value in args.expert)
        if actual_experts != reference["experts"]:
            raise ValueError("Paired experts differ")
        for path, key in ((args.prior_model, "prior_model"), (args.base_model, "base_model")):
            if str(Path(path).resolve()) != reference["inputs"][key]:
                raise ValueError("Paired base/prior differs")
        full_ridges = {key: value["ridge"] for key, value in reference["modules"].items()}
        if any(not math.isfinite(value) or value <= 0 for value in full_ridges.values()):
            raise ValueError("Invalid frozen ridge")
    else:
        raise ValueError("Independent night arm cannot impersonate Table3")
    ...
    def selected_rows(value, name, index, module_name=''):
        ...
        ids = row_indices(len(flat), 16, sample['flow_index'], sample['selected_request_slot'],
                          ...
                          last_only=ablation['variant'] == 'last')
        return flat[ids]
    ...
    if full_prefix:
        ...
        for block_index in range(27):
            ...
            if args.freeze_prefix_from_prior or (ablation is not None and ablation['variant'] == 'conditioning_action'):
                for spec in specs:
    ...
    realized_row_total = validate_realized_rows(metrics) if ablation is None else validate_rows(metrics, names, ablation['variant'])
    ...
    manifest = {
        ...
        "training": False,
        "table1_method": "TCR-E-uniform-frozenridge-development",
        "ablation": ablation,
        "ablation_config_sha256": sha256(args.ablation_config) if args.ablation_config else None,
        "fixed_ridge_reference": str(args.night_reference),
        "night_reference_sha256": sha256(args.night_reference),
