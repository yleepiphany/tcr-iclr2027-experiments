...
COVARIANCE_SCHEMA = "claude_balanced_pass2_v1"
...
def validate_covariance_config(config, bank_sha):
    ...
    if config.get("expert_masses") != "balanced_pass2":
        raise ValueError("Balanced pass2 declares its own mass rule")
    if config.get("pass_index") != 2:
        raise ValueError("This contract is the second calibration pass")
    if config.get("second_pass_inputs") not in ("expert2", "mixed2"):
        raise ValueError("Unknown second-pass input source")
    # In pass 1 the start point (Soup) and the ridge source (tcre-r1) were different
    # checkpoints. In pass 2 BOTH are N0: the regularisation centre moves with the
    # sequential recipe, and that is recorded rather than glossed as "same recipe, one
    # more pass".
    if config.get("start_point_sha256") != config.get("ridge_source_sha256_model"):
        raise ValueError("Pass 2 requires the start point and the ridge centre to be N0")
    ...
    if Path(config["start_point"]).resolve() != Path(
        ...
        raise ValueError("Pass 2 takes its ridges from the start point's own manifest")
    # Pass 2 deliberately differs by arm: expert2 is pure-expert, mixed2 takes slots
    # 1/2/3 from N0's own execution. The contract must therefore pin the slots each arm
    # declares, not forbid mixing outright - forbidding it is the pass-1 rule and would
    # reject the very arm this study exists to build.
    declared = config.get("merged_slots")
    expected_slots = [] if arm == "expert2" else [1, 2, 3]
    if declared != expected_slots:
        raise ValueError(
            f"Arm {arm} must declare merged_slots {expected_slots}, config says {declared}")
...
def main() -> None:
    ...
    manifest = {
        ...
        "balanced_pass2_arm": (ablation.get('arm')
                               if ablation and ablation.get('schema') == COVARIANCE_SCHEMA
                               else None),
        "second_pass_inputs": (ablation.get('second_pass_inputs')
                               if ablation and ablation.get('schema') == COVARIANCE_SCHEMA
                               else None),
        "regularisation_centre_note": (
            "pass 2 recentres the ridge on N0; pass 1 used Soup. The pass1/pass2 contrast "
            "is therefore NOT 'only the number of passes'."
