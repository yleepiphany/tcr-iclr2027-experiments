...
BASE_OFFSET = 30
# The repaired LIBERO tree registered for 2026-09-19. The deleted "Jenson" tree must not
# be inherited, so this is pinned rather than defaulted.
LIBERO_ROOT = Path(__file__).resolve().parents[3] / ".datasets/LIBERO/20260919"
LIBERO_CONFIG = LIBERO_ROOT / "config-standard"


def bind_libero_runtime():
    """Pin the LIBERO config and asset cache before anything imports libero.

    Adapted read-only from `vla-merge/scripts/tcr_night_worker_20260919.py` (frozen, not
    modified).  That worker sets these in-process because `libero.libero` caches its
    asset path at import time, so an env var alone is not enough.  Verifying instead of
    silently defaulting means a run against the wrong asset tree fails closed rather than
    producing forty plausible but incomparable episodes.
    """
    import yaml
    configured = os.environ.get("LIBERO_CONFIG_PATH")
    if configured is None:
        os.environ["LIBERO_CONFIG_PATH"] = configured = str(LIBERO_CONFIG)
    if Path(configured).resolve() != LIBERO_CONFIG.resolve():
        raise ValueError(f"LIBERO_CONFIG_PATH is {configured}, not the registered "
                         f"{LIBERO_CONFIG}")
    for key in ("LIBERO_PRO_REPO", "LIBERO_PRO_ASSET_DIR", "MUJOCO_EGL_DEVICE_ID"):
        os.environ.pop(key, None)
    config = yaml.safe_load((LIBERO_CONFIG / "config.yaml").read_text())
    for key in ("assets", "bddl_files", "benchmark_root", "init_states"):
        if not Path(config[key]).exists():
            raise ValueError(f"LIBERO {key} missing at {config[key]}")
    import libero.libero as inner
    inner._assets_path_cache = str(config["assets"])  # noqa: SLF001
    return {"config_path": configured, "assets": config["assets"],
...
def main():
    libero_binding = bind_libero_runtime()
    import torch
    ...
    def run_one(task_group, task_id, env, **kwargs):
        ...
        receipt = {"key": current["key"], "suite": task_group, "task_id": task_id,
                   ...
                   "libero_config_path": libero_binding["config_path"],
                   "libero_assets": libero_binding["assets"],
