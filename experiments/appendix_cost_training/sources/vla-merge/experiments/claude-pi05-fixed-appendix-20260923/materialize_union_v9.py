#!/usr/bin/env python3
"""Isolated full-scope solver for the fixed A_new→B_new appendix reference.
...
def main() -> None:
    ...
    if ablation is None or ablation.get('schema') != 'pi05_fixed_appendix_single_a_union_b_v1':
        raise ValueError('This isolated solver accepts only one-pass A union B')
    full_ridges = {}
    if ablation is None:
        ...
        is_union = ablation.get('schema') == union.SCHEMA
        if is_three_level:
            ...
            union.validate_config(ablation)
            pass_index = 1
            recipe = {"row_cap": 16, "normalization": "prior"}
        else:
        ...
        if is_three_level or is_matched_main or is_subset or is_appendix or is_union:
        ...
        if is_three_level or is_matched_main or is_subset or is_appendix or is_union:
            ...
            ridge_source = (ablation.get('ridge_source_manifest') if is_three_level or is_subset or is_appendix or is_union
    ...
    union.validate_sources(calibrations, manifests)
        ...
        for calibration_path, manifest_path, source_weight in zip(
            ...
            if ablation is not None and ablation.get('schema') == three_level.SCHEMA:
                ...
                appendix.validate_trace(
                    manifest, dense_bank[name]["path"], name,
                    int(ablation['pass_index']))
            elif ablation is not None and ablation.get('schema') == union.SCHEMA:
                union.validate_trace(manifest, Path(dense_bank[name]["path"]), name)
            else:
    ...
    union_samples_by_name = {
        name: [sample for manifest in manifests_by_name[name]
               for sample in manifest['samples']]
        for name in names
    }

    def selected_rows(value, name, index, module_name=''):
        if ablation is not None and ablation.get('schema') == union.SCHEMA:
            return union.select_rows(value, index, module_name,
                                     union_samples_by_name[name],
    ...
    elif ablation.get('schema') == union.SCHEMA:
        realized_row_total = union.validate_rows(metrics, names)
    else:
    ...
    manifest = {
        ...
        "fixed_ridge_reference": (
            ...
            if ablation and ablation.get('schema') in (three_level.SCHEMA, subset.SCHEMA, appendix.SCHEMA, union.SCHEMA)
            else ablation.get('full_manifest')
        ...
        "fixed_appendix_arm": (
            ablation.get('arm') if ablation and ablation.get('schema') in (appendix.SCHEMA, union.SCHEMA) else None),
        "fixed_appendix_pass": (
            ablation.get('pass_index') if ablation and ablation.get('schema') in (appendix.SCHEMA, union.SCHEMA) else None),
