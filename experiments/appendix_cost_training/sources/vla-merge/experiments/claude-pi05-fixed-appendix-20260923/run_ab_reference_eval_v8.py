#!/usr/bin/env python3
"""Formal repeat-01 400-episode evaluation of the shared A_new→B_new reference."""
...
BUILDS = ROOT / "ab-reference-build-attempt-02"
RUN = ROOT / "ab-reference-formal-eval-attempt-02"
...
def prepare() -> dict:
    ...
    if (builds.get("schema") != "pi05_fixed_appendix_ab_build_queue_v1"
            or [row.get("id") for row in builds.get("jobs", [])] != ["A", "AB"]
            or selection["tasks"] != 40 or selection["eval_seed"] != 274001
            or sha(Path(selection["selection"])) != selection["sha256"]):
        raise ValueError("Appendix AB build or formal reset bank differs")
    ...
    for name in appendix.SUITES:
        job = {"id": f"AB-{name}", "suite": name, "full_suite": SUITE_NAMES[name],
               "checkpoint": str(checkpoint()), "needs_build": "AB",
               "output": str(RUN / "jobs" / f"AB-{name}"),
               "episodes": 100, "repeat": 1}
        job["command"] = make_command(job, selection)
        jobs.append(job)
    if len(jobs) != 4 or len({row["id"] for row in jobs}) != 4:
        raise ValueError("AB formal evaluation coverage differs")
    frozen = {"schema": "pi05_fixed_appendix_ab_formal_eval_v1",
              ...
              "paper_scope": "shared A_new->B_new fixed appendix reference n=1; four suites x ten tasks x ten episodes; not historical Full"}
...
def validate_plan(plan: dict) -> None:
    selection = formal_contract.selection_for(1)
    if (plan.get("schema") != "pi05_fixed_appendix_ab_formal_eval_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != [f"AB-{name}" for name in appendix.SUITES]
...
def verify_checkpoint(job: dict, builds: dict) -> str:
    ...
    if (data.get("fixed_appendix_arm") != "AB"
        ...
        raise ValueError("AB model identity/rows differ")
    if sha(model) != data["model_sha256"]:
        raise ValueError("AB model file changed")
...
def run() -> None:
    ...
    if (RUN / "started.json").exists():
        raise ValueError("No restart within this formal AB reference attempt")
    validate_plan(plan)
        ...
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                ...
                if need in builds.get("failed", {}) or need in builds.get("skipped", {}):
                    blocked[job_id] = "Required AB reference build failed"
                    pending.pop(job_id)
            if not stop:
                ...
                if ready and len(active) < MAX_ACTIVE:
                    ...
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if gpu in {item["gpu"] for item in active.values()} or row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-appendix-ab-formal-eval")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-appendix-ab-formal-eval"})
                        again = gpu_rows()[gpu]
