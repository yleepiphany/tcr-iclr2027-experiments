#!/usr/bin/env python3
"""No-retry, GPU-lease-safe early-request A→B fixed appendix build queue."""
...
RUN = ROOT / "early-build-attempt-02"
SOLVER = HERE / "materialize_early_v8.py"
...
EARLY_AUDIT = VLA / "experiments/tcr-mainline-local-20260919"
SOURCE_AUDIT = WORK / "coordination/2026-09-23/appendix-early-pools-content-audit.json"
...
sys.path[:0] = [str(HERE), str(THREE_LEVEL), str(VLA / "scripts"),
                 str(FORMAL), str(EARLY_AUDIT)]
import appendix_early_contract_v8 as contract  # noqa: E402
...
def output_path(pass_index: int) -> Path:
    return ROOT / "checkpoints-v14" / ("early_A" if pass_index == 1 else "early_AB")
...
def source_gate() -> tuple[dict[str, dict], dict[str, dict]]:
    ...
    if (audit.get("accepted") is not True or audit.get("jobs") != 8
            or audit.get("samples") != 1200 or audit.get("heldout_disjoint") is not True
            or budget.get("B_second_pass_rows", {}).get("base") != contract.ROWS[2]
            or budget.get("A_first_pass_rows") != contract.ROWS[1]):
        raise ValueError("early A/B content or row-budget gate differs")
    ...
    audited = {row["id"]: row for row in audit.get("accepted_jobs", [])}
    expected_ids = {f"collect-{pool}-{name}" for pool in contract.POOLS.values()
                    for name in contract.SUITES}
    if set(audited) != expected_ids or len(audit["accepted_jobs"]) != 8:
        raise ValueError("early A/B content audit job set differs")
    bindings = {}
    for pass_index, pool in contract.POOLS.items():
        for name in contract.SUITES:
            cache = contract.INPUTS / pool / "inputs" / name / "early"
            ...
            audited_row = audited[f"collect-{pool}-{name}"]
            if (audited_row.get("pool") != pool or audited_row.get("suite") != name
                    or audited_row.get("manifest_sha256") != sha(manifest)
                    or audited_row.get("tensor_sha256") != sha(replay)):
                raise ValueError(f"early A/B content audit binding differs: {pool}/{name}")
...
def command_for(pass_index: int, experts: dict[str, dict]) -> list[str]:
    start = contract.SOUP if pass_index == 1 else output_path(1)
    config = RUN / "configs" / f"pass-{pass_index}.json"
    output = output_path(pass_index)
    command = [str(PYTHON), "-u", str(SOLVER), f"--ablation-config={config}",
               ...
               f"--prior-model={start}", f"--output={output}",
               "--ridge-ratio=.05", "--ridge-scale=feature_energy",
               "--max-correction-ratio=3", f"--max-rows-per-sample={contract.CAP[pass_index]}",
               "--device=cuda", "--allow-mixed-calibration-policies",
               "--allow-prior-calibration-mismatch",
               f"--expert-loss-normalization={contract.NORMALIZATION[pass_index]}",
               "--replay-prefix=merged"]
    for name in contract.SUITES:
        cache = contract.INPUTS / contract.POOLS[pass_index] / "inputs" / name / "early"
        command.extend([f"--expert={name}={experts[name]['source_adapter']['path']}",
                        f"--calibration={name}={cache / 'replay.safetensors'}",
                        f"--manifest={name}={cache / 'replay.json'}"])
    return command


def prepare() -> dict:
    if RUN.exists() or any(output_path(i).exists() for i in (1, 2)):
        raise FileExistsError("frozen appendix queue/checkpoint already exists")
    experts, bindings = source_gate()
    jobs = [{"id": "early_A", "pass_index": 1, "needs": [],
             "output": str(output_path(1)), "config": str(RUN / "configs/pass-1.json"),
             "command": command_for(1, experts)},
            {"id": "early_AB", "pass_index": 2, "needs": ["early_A"],
             "output": str(output_path(2)), "config": str(RUN / "configs/pass-2.json"),
             "command": command_for(2, experts)}]
    plan = {"schema": "pi05_fixed_appendix_early_build_queue_v1",
            "created_at": datetime.now(timezone.utc).isoformat(), "host": socket.gethostname(),
            "jobs": jobs, "trace_bindings": bindings,
            "source_audit_sha256": sha(SOURCE_AUDIT), "row_budget_sha256": sha(ROW_BUDGET),
            "dense_bank_sha256": sha(contract.DENSE_BANK),
            "solver_sha256": sha(SOLVER), "contract_sha256": sha(HERE / "appendix_early_contract_v8.py"),
            ...
            "paper_scope": "early-request A->B fixed control, one build per pass, 400 formal episodes after artifact acceptance"}
...
def validate_plan(plan: dict) -> None:
    if (plan.get("schema") != "pi05_fixed_appendix_early_build_queue_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != ["early_A", "early_AB"]
            ...
            or plan.get("contract_sha256") != sha(HERE / "appendix_early_contract_v8.py")):
        raise ValueError("appendix build plan or source changed")
    for key, row in plan["trace_bindings"].items():
        if row["manifest_sha256"] != sha(Path(row["manifest"])) or \
            ...
            raise ValueError(f"appendix trace changed: {key}")


def config_for(job: dict, accepted: dict[str, dict]) -> dict:
    pass_index = job["pass_index"]
    if pass_index == 2 and "early_A" not in accepted:
        raise ValueError("AB requires accepted A")
    start = contract.SOUP if pass_index == 1 else output_path(1)
    config = {"schema": contract.SCHEMA,
              "arm": job["id"], "variant": "full", "pass_index": pass_index,
              "pool": contract.POOLS[pass_index],
              "row_cap_per_request_module": contract.CAP[pass_index],
              "expected_realized_rows": contract.ROWS[pass_index],
              "mass_rule": "relative_prior_error" if pass_index == 1 else "uniform_quarter",
              "expert_loss_normalization": contract.NORMALIZATION[pass_index],
              ...
              "start_point": str(start), "start_point_sha256": sha(start / "model.safetensors"),
              "ridge_source_manifest": str(contract.RIDGE),
              "ridge_source_sha256": sha(contract.RIDGE),
              "parent_A_manifest": str(start / "block_regmeanpp_manifest.json") if pass_index == 2 else None}
...
def verify(job: dict) -> dict:
    ...
    if not model.is_file() or not manifest.is_file():
        raise ValueError("solve produced no model and manifest")
    data = json.loads(manifest.read_text())
    modules = data.get("modules") or {}
    rows = contract.validate_rows(modules, list(contract.SUITES), job["pass_index"])
    if (data.get("fixed_appendix_arm") != job["id"]
            or data.get("fixed_appendix_pass") != job["pass_index"]
        ...
        raise ValueError("appendix model identity, coverage or hash differs")
    frozen = json.loads(Path(job["config"]).read_text())
    contract.validate_config(frozen)
    ridges = json.loads(contract.RIDGE.read_text())["modules"]
    if any(modules[name]["ridge"] != entry["ridge"] for name, entry in ridges.items()):
        raise ValueError("appendix numeric ridges differ from frozen source")
    if job["pass_index"] == 2 and frozen["start_point_sha256"] != \
            sha(output_path(1) / "model.safetensors"):
        raise ValueError("AB parent A checkpoint differs")
...
def run() -> None:
    ...
    try:
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                if any(dep in failed or dep in skipped for dep in job["needs"]):
                    skipped[job_id] = "required early-A build failed"
                    pending.pop(job_id)
                        ...
                        try:
                            ...
                            env = {**os.environ, "CUDA_VISIBLE_DEVICES": str(gpu),
                                   ...
                                   "PYTHONPATH": ":".join([str(HERE), str(THREE_LEVEL), str(VLA / "scripts"),
                                                         str(VLA / "experiments/tcr-mainline-local-20260919"),
        ...
        if not active:
            save(RUN / "queue-ended.json", {"complete": len(accepted) == 2 and not failed and not skipped,
