#!/usr/bin/env python3
"""No-retry A→C and A→B→{B,C} appendix cache-follow-on builds."""
...
RUN = ROOT / "cache-followons-build-attempt-02"
PARENT = ROOT / "ab-reference-build-attempt-02"
SOLVER = HERE / "materialize_cache_followons_v8.py"
...
import appendix_cache_followon_contract_v8 as followon  # noqa: E402
...
def output_path(arm: str) -> Path:
    return ROOT / "checkpoints-v11" / arm
...
def source_gate() -> tuple[dict[str, dict], dict[str, dict]]:
    ...
    for pool in ("B_new", "C_new"):
        for name in contract.SUITES:
            ...
            followon.validate_trace(json.loads(manifest.read_text()), dense_path, name, pool)
...
def command_for(arm_name: str, experts: dict[str, dict]) -> list[str]:
    arm = followon.ARMS[arm_name]
    start = followon.parent_path(arm_name)
    config = RUN / "configs" / f"{arm_name}.json"
    output = output_path(arm_name)
    command = [str(PYTHON), "-u", str(SOLVER), f"--ablation-config={config}",
               ...
               "--max-correction-ratio=3", f"--max-rows-per-sample={TIERS[arm['tier']]}",
               "--device=cuda", "--allow-mixed-calibration-policies",
               "--allow-prior-calibration-mismatch",
               "--expert-loss-normalization=none",
               "--replay-prefix=merged"]
    for name in contract.SUITES:
        cache = contract.INPUTS / arm["pool"] / "inputs" / name / "across"
...
def prepare() -> dict:
    if RUN.exists() or any(output_path(name).exists() for name in followon.ARMS):
    ...
    if (parent_plan.get("schema") != "pi05_fixed_appendix_ab_build_queue_v1"
            or [row.get("id") for row in parent_plan.get("jobs", [])] != ["A", "AB"]):
        raise ValueError("A/AB parent build plan differs")
    jobs = [{"id": name, "pass_index": arm["pass_index"], "pool": arm["pool"],
             "row_tier": arm["tier"], "needs_parent": arm["parent"],
             "output": str(output_path(name)),
             "config": str(RUN / "configs" / f"{name}.json"),
             "command": command_for(name, experts)}
            for name, arm in followon.ARMS.items()]
    plan = {"schema": "pi05_fixed_appendix_cache_followons_build_v3",
            "created_at": datetime.now(timezone.utc).isoformat(), "host": socket.gethostname(),
            "jobs": jobs, "trace_bindings": bindings,
            "parent_build_plan_sha256": sha(PARENT / "plan.json"),
            "source_audit_sha256": sha(SOURCE_AUDIT), "row_budget_sha256": sha(ROW_BUDGET),
            "dense_bank_sha256": sha(contract.DENSE_BANK),
            "solver_sha256": sha(SOLVER), "contract_sha256": sha(HERE / "appendix_cache_followon_contract_v8.py"),
            "row_selector_sha256": sha(HERE / "appendix_row_budget.py"),
            ...
            "paper_scope": "A->C, A->B->B, A->B->C; fixed B/C independent expert caches, same A-origin numerical ridge, each gets 400 formal episodes after artifact acceptance"}
...
def validate_plan(plan: dict) -> None:
    if (plan.get("schema") != "pi05_fixed_appendix_cache_followons_build_v3"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != list(followon.ARMS)
            or plan.get("parent_build_plan_sha256") != sha(PARENT / "plan.json")
            ...
            or plan.get("contract_sha256") != sha(HERE / "appendix_cache_followon_contract_v8.py")
            or plan.get("row_selector_sha256") != sha(HERE / "appendix_row_budget.py")):
...
def config_for(job: dict, parent: dict) -> dict:
    if job["needs_parent"] not in parent.get("accepted", {}):
        raise ValueError("cache follow-on requires its accepted external parent")
    arm = followon.ARMS[job["id"]]
    start = followon.parent_path(job["id"])
    config = {"schema": followon.SCHEMA,
              "arm": job["id"], "variant": "full", "pass_index": arm["pass_index"],
              "pool": arm["pool"], "row_tier": arm["tier"],
              "row_cap_per_request_module": TIERS[arm["tier"]],
              "expected_realized_rows": contract.ROWS[2],
              ...
              "parent_manifest": str(start / "block_regmeanpp_manifest.json")}
    followon.validate_config(config)
...
def verify(job: dict) -> dict:
    ...
    rows = followon.validate_rows(modules, list(contract.SUITES), job["id"])
    if (data.get("fixed_appendix_followon_arm") != job["id"]
            or data.get("fixed_appendix_followon_tier") != job["row_tier"]
    ...
    followon.validate_config(frozen)
    ...
    if frozen["start_point_sha256"] != sha(followon.parent_path(job["id"]) / "model.safetensors"):
        raise ValueError("cache follow-on parent checkpoint differs")
...
def run() -> None:
    ...
    try:
        while pending or active:
            ...
            for job_id, item in list(active.items()):
                ...
                if result:
                    ...
                    failed[job_id] = error
            parent = json.loads((PARENT / "state.json").read_text())
            for job_id, job in list(pending.items()):
                if job["needs_parent"] in parent.get("failed", {}) or job["needs_parent"] in parent.get("skipped", {}):
                    skipped[job_id] = "required external A/AB parent build failed"
                    pending.pop(job_id)
            if not stop and not active:
                ready = [job for job in pending.values()
                         if job["needs_parent"] in parent.get("accepted", {})]
                if ready:
                    rows = gpu_rows()
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-fixed-appendix-followon")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-fixed-appendix-followon"})
                        if legacy is None:
        ...
        if not active:
            save(RUN / "queue-ended.json", {"complete": len(accepted) == 3 and not failed and not skipped,
