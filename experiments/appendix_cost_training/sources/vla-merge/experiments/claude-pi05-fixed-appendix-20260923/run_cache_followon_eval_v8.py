#!/usr/bin/env python3
"""Formal repeat-01 400 episodes each for AC, ABB and ABC cache contrasts."""
...
BUILDS = ROOT / "cache-followons-build-attempt-02"
RUN = ROOT / "cache-followons-formal-eval-attempt-02"
...
sys.path[:0] = [str(HERE), str(FORMAL), str(FLOCKS), str(THREE_LEVEL), str(VLA / "scripts")]
...
def prepare() -> dict:
    ...
    if (builds.get("schema") != "pi05_fixed_appendix_cache_followons_build_v3"
            or [row.get("id") for row in builds.get("jobs", [])] != list(followon.ARMS)
            or selection["tasks"] != 40 or selection["eval_seed"] != 274001
            or sha(Path(selection["selection"])) != selection["sha256"]):
        raise ValueError("Appendix cache-follow-on builds or formal reset bank differ")
    ...
    for arm_name in followon.ARMS:
        for name in appendix.SUITES:
            ...
            jobs.append(job)
    if len(jobs) != 12 or len({row["id"] for row in jobs}) != 12:
        raise ValueError("Cache-follow-on formal evaluation coverage differs")
    frozen = {"schema": "pi05_fixed_appendix_cache_followons_formal_eval_v1",
              ...
              "formal_repeat": 1, "formal_episodes": 1200,
              ...
              "paper_scope": "A->C, A->B->B, A->B->C; one accepted checkpoint and 400 formal episodes each, same repeat-01 selection; not historical Full"}
...
def validate_plan(plan: dict) -> None:
    selection = formal_contract.selection_for(1)
    if (plan.get("schema") != "pi05_fixed_appendix_cache_followons_formal_eval_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != [
                f"{arm}-{suite}" for arm in followon.ARMS for suite in appendix.SUITES]
            or sum(job["episodes"] for job in plan["jobs"]) != 1200
        ...
        raise ValueError("Frozen appendix cache-follow-on evaluation plan changed")


def verify_checkpoint(job: dict, builds: dict) -> str:
    if job["needs_build"] not in builds.get("accepted", {}):
        raise ValueError("cache-follow-on build is not accepted")
    root = Path(job["checkpoint"])
    model = root / "model.safetensors"
    manifest = root / "block_regmeanpp_manifest.json"
    if not model.is_file() or not manifest.is_file():
        raise ValueError("cache-follow-on checkpoint is missing")
    data = json.loads(manifest.read_text())
    if (data.get("fixed_appendix_followon_arm") != job["arm"]
            or data.get("fixed_appendix_followon_tier") != followon.ARMS[job["arm"]]["tier"]
            or data.get("realized_row_total") != appendix.ROWS[2]
            or data.get("model_sha256") != builds["accepted"][job["needs_build"]]["model_sha256"]):
        raise ValueError("cache-follow-on model identity/rows differ")
    if sha(model) != data["model_sha256"]:
        raise ValueError("cache-follow-on model file changed")
...
def run() -> None:
    ...
    if (RUN / "started.json").exists():
        raise ValueError("No restart within this formal cache-follow-on attempt")
    validate_plan(plan)
    ...
    try:
        while pending or active:
            ...
            builds = json.loads((BUILDS / "state.json").read_text())
            ...
            for job_id, job in list(pending.items()):
                ...
                if need in builds.get("failed", {}) or need in builds.get("skipped", {}):
                    blocked[job_id] = "Required cache-follow-on build failed"
                    pending.pop(job_id)
            if not stop:
                ...
                if ready and len(active) < MAX_ACTIVE:
                    ...
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if gpu in {item["gpu"] for item in active.values()} or row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-appendix-cache-followon-formal-eval")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-appendix-cache-followon-formal-eval"})
                        again = gpu_rows()[gpu]
        ...
        if not active:
            save(RUN / "queue-ended.json", {"complete": len(accepted) == 12 and not failed and not blocked,
