#!/usr/bin/env python3
"""Isolated full-scope solver for A→C and third-pass B/C cache controls.
...
def main() -> None:
    ...
    if ablation is None or ablation.get('schema') != 'pi05_fixed_appendix_cache_followon_v3':
        raise ValueError('This isolated solver accepts only registered B/C cache follow-ons')
    full_ridges = {}
    if ablation is None:
        ...
        import appendix_cache_followon_contract_v8 as followon
        from appendix_row_budget import TIERS, nested_row_indices
        ...
        is_followon = ablation.get('schema') == followon.SCHEMA
        if is_three_level:
            ...
            followon.validate_config(ablation)
            pass_index = 2
            recipe = {"row_cap": TIERS[ablation['row_tier']],
                      "normalization": "none"}
        else:
        ...
        if is_three_level or is_matched_main or is_subset or is_appendix or is_followon:
        ...
        if is_three_level or is_matched_main or is_subset or is_appendix or is_followon:
            ...
            ridge_source = (ablation.get('ridge_source_manifest') if is_three_level or is_subset or is_appendix or is_followon
        ...
        for calibration_path, manifest_path, source_weight in zip(
            ...
            if ablation is not None and ablation.get('schema') == three_level.SCHEMA:
                ...
                appendix.validate_trace(
                    manifest, dense_bank[name]["path"], name,
                    int(ablation['pass_index']))
            elif ablation is not None and ablation.get('schema') == followon.SCHEMA:
                followon.validate_trace(
    ...
    def selected_rows(value, name, index, module_name=''):
        ...
        if ablation.get('schema') == followon.SCHEMA and ablation['row_tier'] != 'base':
            ids = nested_row_indices(len(flat), ablation['row_tier'],
    ...
    elif ablation.get('schema') == followon.SCHEMA:
        realized_row_total = followon.validate_rows(metrics, names, ablation['arm'])
    else:
    ...
    manifest = {
        ...
        "fixed_ridge_reference": (
            ...
            if ablation and ablation.get('schema') in (three_level.SCHEMA, subset.SCHEMA, appendix.SCHEMA, followon.SCHEMA)
            else ablation.get('full_manifest')
        ...
        "fixed_appendix_followon_arm": (
            ablation.get('arm') if ablation and ablation.get('schema') == followon.SCHEMA else None),
        "fixed_appendix_followon_tier": (
            ablation.get('row_tier') if ablation and ablation.get('schema') == followon.SCHEMA else None),
