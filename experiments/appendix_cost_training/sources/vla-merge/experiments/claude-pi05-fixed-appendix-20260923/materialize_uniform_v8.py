#!/usr/bin/env python3
"""Isolated full-scope solver for the uniform-first-pass appendix control.
...
def main() -> None:
    ...
    if ablation is None or ablation.get('schema') != 'pi05_fixed_appendix_uniform_first_v4':
        raise ValueError('This isolated solver accepts only the uniform-first-pass control')
    full_ridges = {}
    if ablation is None:
        ...
        import appendix_uniform_contract_v8 as appendix
        ...
        if is_three_level:
            ...
            appendix.validate_config(ablation)
            pass_index = int(ablation['pass_index'])
            recipe = {"row_cap": 10 if pass_index == 1 else 16,
                      "normalization": "none"}
        else:
    ...
    if ablation is None:
        ...
        realized_row_total = appendix.validate_rows(metrics, names, ablation['arm'])
    else:
