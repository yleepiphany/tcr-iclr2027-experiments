#!/usr/bin/env python3
"""Formal repeat-01 400-episode evaluation of the shared A_new first pass."""
...
RUN = ROOT / "a-reference-formal-eval-attempt-02"
...
def prepare() -> dict:
    ...
    if (builds.get("schema") != "pi05_fixed_appendix_ab_build_queue_v1"
        ...
        raise ValueError("Appendix A build or formal reset bank differs")
    ...
    for name in appendix.SUITES:
        job = {"id": f"A-{name}", "suite": name, "full_suite": SUITE_NAMES[name],
               "checkpoint": str(checkpoint()), "needs_build": "A",
               "output": str(RUN / "jobs" / f"A-{name}"),
               "episodes": 100, "repeat": 1}
        job["command"] = make_command(job, selection)
        jobs.append(job)
    if len(jobs) != 4 or len({row["id"] for row in jobs}) != 4:
        raise ValueError("A formal evaluation coverage differs")
    frozen = {"schema": "pi05_fixed_appendix_a_formal_eval_v1",
              ...
              "paper_scope": "shared A_new first-pass fixed appendix reference n=1; four suites x ten tasks x ten episodes; not historical Full"}
...
def validate_plan(plan: dict) -> None:
    selection = formal_contract.selection_for(1)
    if (plan.get("schema") != "pi05_fixed_appendix_a_formal_eval_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != [f"A-{name}" for name in appendix.SUITES]
...
def run() -> None:
    ...
    if (RUN / "started.json").exists():
        raise ValueError("No restart within this formal A reference attempt")
    validate_plan(plan)
        ...
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                ...
                if need in builds.get("failed", {}) or need in builds.get("skipped", {}):
                    blocked[job_id] = "Required A reference build failed"
                    pending.pop(job_id)
            if not stop:
