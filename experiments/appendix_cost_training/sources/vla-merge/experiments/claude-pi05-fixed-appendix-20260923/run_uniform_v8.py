#!/usr/bin/env python3
"""No-retry uniform-first-pass fixed appendix build queue."""
...
RUN = ROOT / "uniform-build-attempt-02"
SOLVER = HERE / "materialize_uniform_v8.py"
...
import appendix_uniform_contract_v8 as uniform  # noqa: E402
...
def output_path(pass_index: int) -> Path:
    return ROOT / "checkpoints-v9" / ("uniform_A" if pass_index == 1 else "uniform_AB")
...
def prepare() -> dict:
    ...
    plan = {"schema": "pi05_fixed_appendix_uniform_build_v4",
            ...
            "solver_sha256": sha(SOLVER), "contract_sha256": sha(HERE / "appendix_uniform_contract_v8.py"),
            ...
            "paper_scope": "uniform expert masses only in A_new first pass; B_new second pass unchanged uniform; one final checkpoint with 400 formal episodes after artifact acceptance"}
...
def validate_plan(plan: dict) -> None:
    if (plan.get("schema") != "pi05_fixed_appendix_uniform_build_v4"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != ["uniform_A", "uniform_AB"]
            ...
            or plan.get("contract_sha256") != sha(HERE / "appendix_uniform_contract_v8.py")):
...
def config_for(job: dict, accepted: dict[str, dict]) -> dict:
    ...
    if pass_index == 2 and "uniform_A" not in accepted:
        raise ValueError("uniform_AB requires accepted uniform_A")
    start = contract.SOUP if pass_index == 1 else output_path(1)
    config = {"schema": uniform.SCHEMA,
...
def verify(job: dict) -> dict:
    ...
    rows = uniform.validate_rows(modules, list(contract.SUITES), job["id"])
    ...
    uniform.validate_config(frozen)
    ...
    if job["pass_index"] == 2 and frozen["start_point_sha256"] != \
        ...
        raise ValueError("uniform_AB parent uniform_A checkpoint differs")
        ...
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                if any(dep in failed or dep in skipped for dep in job["needs"]):
                    skipped[job_id] = "required uniform_A build failed"
                    pending.pop(job_id)
                ...
                if ready:
                    ...
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-fixed-appendix-uniform")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-fixed-appendix-uniform"})
                        if legacy is None:
