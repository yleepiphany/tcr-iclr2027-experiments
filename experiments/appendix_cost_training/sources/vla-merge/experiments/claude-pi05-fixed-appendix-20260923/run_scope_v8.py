#!/usr/bin/env python3
"""No-retry action-only and conditioning+action fixed appendix builds."""
...
RUN = ROOT / "scope-build-attempt-02"
SOLVER = HERE / "materialize_scope_v8.py"
...
def output_path(arm_name: str) -> Path:
    return ROOT / "checkpoints-v12" / arm_name
...
def command_for(arm_name: str, experts: dict[str, dict]) -> list[str]:
    ...
    pass_index = arm["pass_index"]
    start = scope.parent_path(arm_name)
    ...
    for name in contract.SUITES:
        cache = contract.INPUTS / contract.POOLS[pass_index] / "inputs" / name / "across"
...
def prepare() -> dict:
    if RUN.exists() or any(output_path(name).exists() for name in scope.ARMS):
        raise FileExistsError("frozen appendix queue/checkpoint already exists")
    experts, bindings = source_gate()
    jobs = [{"id": name, "pass_index": arm["pass_index"], "scope": arm["scope"],
             "needs": [] if arm["pass_index"] == 1 else [arm["parent"]],
             "output": str(output_path(name)),
             "config": str(RUN / "configs" / f"{name}.json"),
             "command": command_for(name, experts)}
            for name, arm in scope.ARMS.items()]
    plan = {"schema": "pi05_fixed_appendix_scope_build_v5",
            ...
            "solver_sha256": sha(SOLVER), "contract_sha256": sha(HERE / "appendix_scope_contract_v8.py"),
            ...
            "paper_scope": "action-only and conditioning+action; two passes per scope, same A/B pools and fixed ridge, final 400 episodes per scope after artifact acceptance"}
...
def validate_plan(plan: dict) -> None:
    if (plan.get("schema") != "pi05_fixed_appendix_scope_build_v5"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != list(scope.ARMS)
            ...
            or plan.get("contract_sha256") != sha(HERE / "appendix_scope_contract_v8.py")):
...
def config_for(job: dict, accepted: dict[str, dict]) -> dict:
    arm = scope.ARMS[job["id"]]
    pass_index = job["pass_index"]
    if pass_index == 2 and arm["parent"] not in accepted:
        raise ValueError("scope B requires accepted matching scope A")
    start = scope.parent_path(job["id"])
    config = {"schema": scope.SCHEMA,
              "arm": job["id"], "variant": arm["scope"], "scope": arm["scope"],
              "pass_index": pass_index, "parent_arm": arm["parent"],
              "pool": contract.POOLS[pass_index],
              "row_cap_per_request_module": contract.CAP[pass_index],
              "expected_realized_rows": scope.ROWS[(arm["scope"], pass_index)],
              ...
              "calibrated_module_count": scope.SCOPE_MODULES[arm["scope"]],
              ...
              "parent_manifest": str(start / "block_regmeanpp_manifest.json") if pass_index == 2 else None}
...
def verify(job: dict) -> dict:
    ...
    rows = scope.validate_rows(modules, list(contract.SUITES), job["id"])
    ...
    scope.validate_config(frozen)
    ridges = json.loads(contract.RIDGE.read_text())["modules"]
    if any(modules[name]["ridge"] != entry["ridge"]
           for name, entry in ridges.items() if modules[name].get("kind") != "frozen_prior"):
        raise ValueError("appendix numeric ridges differ from frozen source")
    if job["pass_index"] == 2 and frozen["start_point_sha256"] != \
            sha(scope.parent_path(job["id"]) / "model.safetensors"):
        raise ValueError("scope B parent scope A checkpoint differs")
...
def run() -> None:
    ...
    try:
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                if any(dep in failed or dep in skipped for dep in job["needs"]):
                    skipped[job_id] = "required matching scope-A build failed"
                    pending.pop(job_id)
            if failed and not active:
                for job_id in list(pending):
                    skipped[job_id] = "first technical failure stopped this no-retry queue"
                    pending.pop(job_id)
                ...
                if ready:
                    ...
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-fixed-appendix-scope")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-fixed-appendix-scope"})
                        if legacy is None:
        ...
        if not active:
            save(RUN / "queue-ended.json", {"complete": len(accepted) == 4 and not failed and not skipped,
