#!/usr/bin/env python3
"""No-retry, GPU-lease-safe A_new→B_new shared appendix build queue."""
...
RUN = ROOT / "ab-reference-build-attempt-02"
SOLVER = HERE / "materialize_fixed_appendix_v8.py"
...
def output_path(pass_index: int) -> Path:
    return ROOT / "checkpoints-v8" / ("A" if pass_index == 1 else "AB")
...
def prepare() -> dict:
    ...
    plan = {"schema": "pi05_fixed_appendix_ab_build_queue_v1",
            ...
            "solver_sha256": sha(SOLVER), "contract_sha256": sha(HERE / "appendix_contract.py"),
            ...
            "paper_scope": "recovery of source path type failure in attempt-01; unchanged shared A_new->B_new recipe, one build per pass"}
...
def validate_plan(plan: dict) -> None:
    if (plan.get("schema") != "pi05_fixed_appendix_ab_build_queue_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != ["A", "AB"]
            ...
            or plan.get("contract_sha256") != sha(HERE / "appendix_contract.py")):
...
def config_for(job: dict, accepted: dict[str, dict]) -> dict:
    ...
    if pass_index == 2 and "A" not in accepted:
