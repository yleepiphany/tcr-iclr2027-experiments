#!/usr/bin/env python3
"""No-retry, GPU-lease-safe A-parent fixed appendix follow-on builds."""
...
RUN = ROOT / "a-followons-build-attempt-02"
PARENT = ROOT / "ab-reference-build-attempt-02"
SOLVER = HERE / "materialize_followons_v8.py"
...
import appendix_followon_contract_v8 as followon  # noqa: E402
...
def output_path(arm: str) -> Path:
    return ROOT / "checkpoints-v10" / arm
...
def prepare() -> dict:
    ...
    jobs = [{"id": name, "pass_index": 2, "pool": arm["pool"],
             "row_tier": arm["tier"], "needs_parent": "A",
             "output": str(output_path(name)),
             "config": str(RUN / "configs" / f"{name}.json"),
             "command": command_for(name, experts)}
            for name, arm in followon.ARMS.items()]
    plan = {"schema": "pi05_fixed_appendix_a_followons_build_v2",
            ...
            "solver_sha256": sha(SOLVER), "contract_sha256": sha(HERE / "appendix_followon_contract_v8.py"),
            ...
            "paper_scope": "A->A, A->B low, A->B high fixed appendix controls; all require the exact accepted A parent; each gets 400 formal episodes after artifact acceptance"}
...
def validate_plan(plan: dict) -> None:
    if (plan.get("schema") != "pi05_fixed_appendix_a_followons_build_v2"
            ...
            or plan.get("contract_sha256") != sha(HERE / "appendix_followon_contract_v8.py")
...
def config_for(job: dict, parent: dict) -> dict:
    if "A" not in parent.get("accepted", {}):
        raise ValueError("follow-on requires accepted external A")
    arm = followon.ARMS[job["id"]]
    start = followon.A
    config = {"schema": followon.SCHEMA,
              "arm": job["id"], "variant": "full", "pass_index": 2,
              ...
              "parent_A_manifest": str(start / "block_regmeanpp_manifest.json")}
...
def verify(job: dict) -> dict:
    ...
    if frozen["start_point_sha256"] != sha(followon.A / "model.safetensors"):
        raise ValueError("follow-on parent A checkpoint differs")
    ...
    try:
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                if "A" in parent.get("failed", {}) or "A" in parent.get("skipped", {}):
                    skipped[job_id] = "required external A build failed"
                    pending.pop(job_id)
