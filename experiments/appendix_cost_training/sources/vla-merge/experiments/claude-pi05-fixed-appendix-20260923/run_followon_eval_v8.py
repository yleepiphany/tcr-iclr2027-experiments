#!/usr/bin/env python3
"""Formal repeat-01 400 episodes each for three A-parent appendix follow-ons."""
...
BUILDS = ROOT / "a-followons-build-attempt-02"
RUN = ROOT / "a-followons-formal-eval-attempt-02"
...
def prepare() -> dict:
    ...
    if (builds.get("schema") != "pi05_fixed_appendix_a_followons_build_v2"
        ...
        raise ValueError("Appendix follow-on builds or formal reset bank differ")
    ...
    if len(jobs) != 12 or len({row["id"] for row in jobs}) != 12:
        raise ValueError("Follow-on formal evaluation coverage differs")
    frozen = {"schema": "pi05_fixed_appendix_a_followons_formal_eval_v1",
              ...
              "paper_scope": "A->A, A->B low, A->B high; one accepted checkpoint and 400 formal episodes each, same repeat-01 selection; not historical Full"}
...
def validate_plan(plan: dict) -> None:
    selection = formal_contract.selection_for(1)
    if (plan.get("schema") != "pi05_fixed_appendix_a_followons_formal_eval_v1"
...
def run() -> None:
    ...
    if (RUN / "started.json").exists():
        raise ValueError("No restart within this formal follow-on attempt")
    validate_plan(plan)
        ...
        while pending or active:
            ...
            for job_id, job in list(pending.items()):
                ...
                if need in builds.get("failed", {}) or need in builds.get("skipped", {}):
                    blocked[job_id] = "Required follow-on build failed"
                    pending.pop(job_id)
            if not stop:
                ...
                if ready and len(active) < MAX_ACTIVE:
                    ...
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if gpu in {item["gpu"] for item in active.values()} or row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-appendix-followon-formal-eval")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-appendix-followon-formal-eval"})
                        again = gpu_rows()[gpu]
