#!/usr/bin/env python3
"""Formal repeat-01 A∪B evaluation after audited metadata recovery."""
...
BUILDS = ROOT / "union-build-artifact-recovery-attempt-03"
RUN = ROOT / "union-formal-eval-attempt-03"
...
def checkpoint() -> Path:
    return ROOT / "checkpoints-v13/A_union_B"
...
def prepare() -> dict:
    ...
    recovery = json.loads((BUILDS / "queue-ended.json").read_text())
    if (recovery.get("complete") is not True
            or recovery.get("reused_model_without_recompute") is not True
            or set(recovery.get("accepted", {})) != {union.ARM}
            or recovery.get("ridge_modules_exact") != 418
            or builds.get("original_artifacts_unchanged") is not True):
        raise ValueError("A∪B model metadata recovery was not independently accepted")
    selection = formal_contract.selection_for(1)
    if (builds.get("schema") != "pi05_fixed_appendix_union_build_queue_v1"
            or [row.get("id") for row in builds.get("jobs", [])] != [union.ARM]
            or selection["tasks"] != 40 or selection["eval_seed"] != 274001
            or sha(Path(selection["selection"])) != selection["sha256"]):
        raise ValueError("Appendix one-pass union build or formal reset bank differs")
    ...
    for name in appendix.SUITES:
        job = {"id": f"{union.ARM}-{name}", "suite": name, "full_suite": SUITE_NAMES[name],
               "checkpoint": str(checkpoint()), "needs_build": union.ARM,
               "output": str(RUN / "jobs" / f"{union.ARM}-{name}"),
               "episodes": 100, "repeat": 1}
        job["command"] = make_command(job, selection)
        jobs.append(job)
    if len(jobs) != 4 or len({row["id"] for row in jobs}) != 4:
        raise ValueError("one-pass union formal evaluation coverage differs")
    frozen = {"schema": "pi05_fixed_appendix_union_formal_eval_v1",
              "created_at": datetime.now(timezone.utc).isoformat(), "host": socket.gethostname(),
              "build_plan_sha256": sha(BUILDS / "plan.json"),
              "build_recovery_receipt_sha256": sha(BUILDS / "queue-ended.json"),
              ...
              "recovery_of": "attempt-02 build passed numerical audit; original failed metadata gate retained; model bytes unchanged",
              "paper_scope": "one-pass A_new union B_new fixed appendix control n=1; four suites x ten tasks x ten episodes; not historical Full"}
...
def validate_plan(plan: dict) -> None:
    selection = formal_contract.selection_for(1)
    if (plan.get("schema") != "pi05_fixed_appendix_union_formal_eval_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != [f"{union.ARM}-{name}" for name in appendix.SUITES]
            or sum(job["episodes"] for job in plan["jobs"]) != 400
            or plan.get("build_plan_sha256") != sha(BUILDS / "plan.json")
            or plan.get("build_recovery_receipt_sha256") != sha(BUILDS / "queue-ended.json")
...
def run() -> None:
    ...
    if (RUN / "started.json").exists():
        raise ValueError("No restart within this formal one-pass union attempt")
    validate_plan(plan)
    ...
    try:
        while pending or active:
            ...
            if not state_path.is_file():
                if stop:
                    raise RuntimeError("Union evaluation supervisor stopped while waiting for build")
                started = BUILDS / "started.json"
                if (BUILDS / "queue-ended.json").exists() or not started.is_file():
                    raise RuntimeError("Union build has no live dependency state")
                ...
                if not proc.is_file() or int(proc.read_text().split()[21]) != owner["kernel_starttime"]:
                    raise RuntimeError("Union build supervisor ended before first state")
            ...
            for job_id, job in list(pending.items()):
                ...
                if need in builds.get("failed", {}) or need in builds.get("skipped", {}):
                    blocked[job_id] = "Required one-pass union build failed"
                    pending.pop(job_id)
            if not stop:
