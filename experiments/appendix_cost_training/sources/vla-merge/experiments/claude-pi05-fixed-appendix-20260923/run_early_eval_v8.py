#!/usr/bin/env python3
"""Formal repeat-01 400-episode evaluation of the early-request A→B control."""
...
BUILDS = ROOT / "early-build-attempt-02"
RUN = ROOT / "early-formal-eval-attempt-02"
...
import appendix_early_contract_v8 as early  # noqa: E402
...
def checkpoint() -> Path:
    return ROOT / "checkpoints-v14/early_AB"
...
def prepare() -> dict:
    ...
    if (builds.get("schema") != "pi05_fixed_appendix_early_build_queue_v1"
            or [row.get("id") for row in builds.get("jobs", [])] != [early.ARMS[1], early.ARMS[2]]
            or selection["tasks"] != 40 or selection["eval_seed"] != 274001
            or sha(Path(selection["selection"])) != selection["sha256"]):
        raise ValueError("Appendix early-request A→B build or formal reset bank differs")
    audit = json.loads((WORK / "coordination/2026-09-23/appendix-early-pools-content-audit.json").read_text())
    if (audit.get("accepted") is not True or audit.get("heldout_disjoint") is not True
            or audit.get("jobs") != 8 or audit.get("samples") != 1200):
        raise ValueError("Early A/B calibration and heldout reset identity gate differs")
    jobs = []
    for name in appendix.SUITES:
        job = {"id": f"{early.ARMS[2]}-{name}", "suite": name, "full_suite": SUITE_NAMES[name],
               "checkpoint": str(checkpoint()), "needs_build": early.ARMS[2],
               "output": str(RUN / "jobs" / f"{early.ARMS[2]}-{name}"),
               "episodes": 100, "repeat": 1}
        job["command"] = make_command(job, selection)
        jobs.append(job)
    if len(jobs) != 4 or len({row["id"] for row in jobs}) != 4:
        raise ValueError("early-request A→B formal evaluation coverage differs")
    frozen = {"schema": "pi05_fixed_appendix_early_formal_eval_v1",
              "created_at": datetime.now(timezone.utc).isoformat(), "host": socket.gethostname(),
              "build_plan_sha256": sha(BUILDS / "plan.json"),
              "input_content_audit_sha256": sha(WORK / "coordination/2026-09-23/appendix-early-pools-content-audit.json"),
              ...
              "formal_repeat": 1, "formal_episodes": 400,
              ...
              "paper_scope": "early-request A_new to B_new fixed appendix control n=1; four suites x ten tasks x ten episodes; not historical Full"}
...
def validate_plan(plan: dict) -> None:
    selection = formal_contract.selection_for(1)
    if (plan.get("schema") != "pi05_fixed_appendix_early_formal_eval_v1"
            or plan.get("host") != socket.gethostname()
            or [job.get("id") for job in plan.get("jobs", [])] != [f"{early.ARMS[2]}-{name}" for name in appendix.SUITES]
            or sum(job["episodes"] for job in plan["jobs"]) != 400
            or plan.get("build_plan_sha256") != sha(BUILDS / "plan.json")
            or plan.get("input_content_audit_sha256") != sha(WORK / "coordination/2026-09-23/appendix-early-pools-content-audit.json")
        ...
        raise ValueError("Frozen appendix early-request evaluation plan changed")


def verify_checkpoint(job: dict, builds: dict) -> str:
    if job["needs_build"] not in builds.get("accepted", {}):
        raise ValueError("early-request A→B build is not accepted")
    root = Path(job["checkpoint"])
    model = root / "model.safetensors"
    manifest = root / "block_regmeanpp_manifest.json"
    if not model.is_file() or not manifest.is_file():
        raise ValueError("early-request A→B checkpoint is missing")
    data = json.loads(manifest.read_text())
    if (data.get("fixed_appendix_arm") != early.ARMS[2]
            or data.get("fixed_appendix_pass") != 2
            or data.get("realized_row_total") != early.ROWS[2]
            or data.get("model_sha256") != builds["accepted"][job["needs_build"]]["model_sha256"]):
        raise ValueError("early-request A→B model identity/rows differ")
    if sha(model) != data["model_sha256"]:
        raise ValueError("early-request A→B model file changed")
...
def run() -> None:
    ...
    if (RUN / "started.json").exists():
        raise ValueError("No restart within this formal early-request A→B attempt")
    validate_plan(plan)
    ...
    try:
        while pending or active:
            state_path = BUILDS / "state.json"
            if not state_path.is_file():
                if stop:
                    raise RuntimeError("Early-request evaluation supervisor stopped while waiting for build")
                started = BUILDS / "started.json"
                if (BUILDS / "queue-ended.json").exists() or not started.is_file():
                    raise RuntimeError("Early-request build has no live dependency state")
                owner = json.loads(started.read_text())
                proc = Path(f"/proc/{owner['pid']}/stat")
                if not proc.is_file() or int(proc.read_text().split()[21]) != owner["kernel_starttime"]:
                    raise RuntimeError("Early-request build supervisor ended before first state")
            ...
            builds = json.loads(state_path.read_text())
            ...
            for job_id, job in list(pending.items()):
                ...
                if need in builds.get("failed", {}) or need in builds.get("skipped", {}):
                    blocked[job_id] = "Required early-request A→B build failed"
                    pending.pop(job_id)
            if not stop:
                ...
                if ready and len(active) < MAX_ACTIVE:
                    ...
                    for gpu in sorted(GPUS, key=lambda idx: rows[idx]["free_mib"], reverse=True):
                        ...
                        if gpu in {item["gpu"] for item in active.values()} or row["free_mib"] < MIN_FREE_MIB:
                            continue
                        job = ready[0]
                        card = card_flock.take_card(gpu, row["uuid"], job["id"], "pi05-appendix-a-formal-eval")
                        if card is None:
                            continue
                        legacy = card_flock._try_lock(
                            RUNTIME / "resource-leases" / socket.gethostname() / f"gpu-{gpu}.lock",
                            job["id"], {"gpu": gpu, "job": job["id"], "stage": "pi05-appendix-a-formal-eval"})
                        again = gpu_rows()[gpu]
        ...
        if not active:
            save(RUN / "queue-ended.json", {"complete": len(accepted) == 4 and not failed and not blocked,
