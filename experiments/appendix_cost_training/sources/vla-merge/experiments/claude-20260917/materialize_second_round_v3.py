...
SECOND_ROUND_ARMS = ("c_e", "c_m", "c_m1", "c_m5", "n1", "n2",
                     "d_mass", "d_rows", "d_orig",
                     # follow-ups: independent first-round builds, a third round, and a
                     # third point on the row-budget curve
                     "r01", "r02", "round3", "d_cap24")
SECOND_ROUND_MASSES = {"uniform_quarter": "none", "relative_error": "prior"}
...
def validate_second_round_config(config):
    ...
    if config.get("row_cap_per_request_module") not in (10, 16, 24) or config.get("repeat") != 1:
        raise ValueError("Second-round budget differs from the registered contract")
    if config.get("expert_masses") not in SECOND_ROUND_MASSES:
        raise ValueError(f"Unknown expert mass rule: {config.get('expert_masses')}")
...
def main() -> None:
    ...
    if ablation is None:
        ...
        if ablation.get('schema') == SECOND_ROUND_SCHEMA:
            # Isolated N1/N2 branch. Same numeric recipe and the same Table-3 row quota,
            # but uniform 1/4 expert masses and numeric ridges frozen from the registered
            # second-round start point instead of the Table-3 Full build.
            validate_second_round_config(ablation)
            expected['replay_prefix'] = 'merged'
            # The decomposition arms vary exactly these two knobs and nothing else.
            expected['expert_loss_normalization'] = SECOND_ROUND_MASSES[ablation['expert_masses']]
            expected['max_rows_per_sample'] = ablation['row_cap_per_request_module']
            for key, value in expected.items():
                if getattr(args, key) != value:
                    raise ValueError(f'Second-round recipe drift: {key}')
            if not args.prior_model:
                raise ValueError('Second round requires the frozen start-point prior')
            if Path(args.prior_model).resolve() != Path(ablation['start_point']).resolve():
                raise ValueError('Prior model is not the registered second-round start point')
    ...
    for name in names:
        ...
        for calibration_path, manifest_path, source_weight in zip(
            ...
            if ablation is not None and ablation.get('schema') == SECOND_ROUND_SCHEMA:
    ...
    def selected_rows(value, name, index, module_name=''):
        ...
        # The Table-3 path hard-codes this cap; the decomposition arms need it to follow
        # the registered config, otherwise a "fewer rows" arm silently reproduces the
        # cap-16 checkpoint bit for bit.
        row_cap = ablation.get('row_cap_per_request_module', 16) if ablation else 16
        ids = row_indices(len(flat), row_cap, sample['flow_index'], sample['selected_request_slot'],
    ...
    # `validate_rows` only knows the cap-16 quota (800 rows per module per expert), so
    # any other registered cap counts its own realised rows instead of being rejected.
    if ablation is not None and ablation.get('row_cap_per_request_module') not in (None, 16):
    ...
    manifest = {
        ...
        "fixed_ridge_reference": (ablation.get('start_point_manifest')
                                  if ablation and ablation.get('schema') == SECOND_ROUND_SCHEMA
