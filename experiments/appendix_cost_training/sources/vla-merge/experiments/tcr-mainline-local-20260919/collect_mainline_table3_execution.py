...
The adapter writes the ``across`` view only.  It retains the complete request sequence
up to the registered cap, then selects five requests from the true observed request
...
def main() -> None:
    ...
    class MainlineCollector(native.BlockCalibrationCollector):
        ...
        def save(self):
            ...
            self.tensor_output = root / "across" / "replay.safetensors"
            self.manifest_output = root / "across" / "replay.json"
            ...
            manifest.update(
                ...
                table3_request_selection="across",
                selected_requests=provenance,
                native_noise_sha256=self.noise_hashes,
                generation_noise_seed=flow_seed,
                selection_rule=(
                    "across quantiles 0, .25, .5, .75, 1 over the true request "
                    "indices; no success filtering"
                ),
                single_repeat=True,
