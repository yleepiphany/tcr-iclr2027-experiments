...
COLLECTOR = HERE / "collect_early_table3_execution.py"
...
FROZEN_SOURCES = (
    HERE / "early_capture_queue_v2.py",
    HERE / "audit_early_requests.py",
...
def prepare_plan(run: Path, bank_path: Path = BANK, max_runtime_hours: float = 13.0) -> dict[str, Any]:
    ...
    plan = {
        ...
        "protocol": {
            ...
            "selection": "first five true observed native requests",
...
def verify_manifest(manifest_path: Path, job: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = {
        ...
        "table3_request_selection": "early", "sample_count": 150,
    ...
    early = check_early(manifest)
    if early["accepted"] is not True:
        raise ValueError(f"{job['id']}: actual early-request samples failed: {early['issues']}")
    return {
    ...
    def worker(gpu: int, slot: int) -> None:
        ...
        while True:
            ...
            if outcome["outcome"] != "success":
                ...
                return
            try:
                receipt = verify_manifest(Path(job["output"]) / "early/replay.json", job)
            except Exception as error:
                with state_lock:
